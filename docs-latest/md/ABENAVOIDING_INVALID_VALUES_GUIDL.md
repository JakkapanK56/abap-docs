...
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENAVOIDING_INVALID_VALUES_GUIDL.html"
...
A [lossless assignment](ABENLOSSLESS_ASSIGNMENT_GLOSRY.md) can be used to force checks on valid values.
...
If the responsibility for filling data objects of the critical data types lies elsewhere, [it is best to](ABENUSE_CONVERSION_RULES_GUIDL.md) always check the validity of their content before use. [Lossless assignments](ABENLOSSLESS_ASSIGNMENT_GLOSRY.md) with the operator [`EXACT`](ABENCONSTRUCTOR_EXPRESSION_EXACT.md) can be used to do this. Note that the initial value of a date field of type `d` is interpreted as valid for a lossless assignment, but the value *00010101* is considered to be a valid date only in [ABAP SQL](ABENABAP_SQL_GLOSRY.md).
