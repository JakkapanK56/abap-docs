...
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENEXCEPTION_TEXTS_GUIDL.html"
...
-   In cases where a class-based exception is used to wrap a [classic exception](ABENCLASS_EXCEPTION_GUIDL.md) and [`MESSAGE ... RAISING`](ABAPMESSAGE_RAISING.md) is used to associate this classic exception with a message text (or after a message is caught using [`error_message`](ABAPCALL_FUNCTION_PARAMETER.md)), the class-based exception can use the same message text regardless of whether the program in question is a system program or application program. The system interface [`IF_T100_DYN_MSG`](ABENIF_T100_DYN_MSG.md) and the addition [`MESSAGE`](ABAPRAISE_EXCEPTION_MESSAGE.md) of the statement [`RAISE EXCEPTION`](ABAPRAISE_EXCEPTION_CLASS.md) and of the addition [`THROW`](ABENCONDITIONAL_EXPRESSION_RESULT.md) in a [conditional exception](ABENCONDITIONAL_EXPRESSIONS.md) are used for these cases.
...
The following source code passes a [UUID](ABENUUID_GLOSRY.md) for an OTR text to the input parameter `TEXTID` of the instance constructor when an exception is raised. According to the rule above, however, only exception texts from the exception class can be passed, with each exception class containing the corresponding constants.
