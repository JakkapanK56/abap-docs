...
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENDATABASE_ACCESS_GUIDL.html"
...
-   **ABAP SQL**
-   Implemented by ABAP statements, [ABAP SQL](ABENABAP_SQL.md) is a subset of the Structured Query Language (SQL) comprising the DQL (Data Query Language) and DML (Data Manipulation Language) parts. The ABAP SQL statements use the ABAP SQL interface for platform-independent access to those database tables in the [standard AS ABAP database](ABENSTANDARD_DB_GLOSRY.md) defined in ABAP Dictionary and having instances created in the [ABAP database schema](ABENABAP_DB_SCHEMA_GLOSRY.md).
-   **AMDP**
...
Alongside access to the ABAP database schema of the standard AS ABAP database, all access types (except for AMDP) also allow access to other databases and other database schemas using additional [database connections](ABENDATABASE_CONNECTION_GLOSRY.md).
...
If the database is accessed using the Native SQL interface instead of the ABAP SQL interface, [AMDP](ABENADBC_GLOSRY.md) or [ADBC](ABENADBC_GLOSRY.md) should be used.
...
-   ADBC is a modern object-oriented API that is better suited to modern ABAP programming than [`EXEC SQL`](ABAPEXEC.md). Enhancements to the Native SQL interface, such as bulk access using internal tables, are now only provided using ADBC. ADBC also enables [dynamic access](ABENDYNAMIC_PROG_TECHNIQUE_GDL.md); Native SQL on the other hand is just static. Newer SQL statements, such as `WITH` for common table expressions (CTEs), are not supported in full by `EXEC SQL`.

For a detailed description, see [Rules for Accessing ABAP-Managed Database Objects](ABENDATABASE_ACCESS_RECOMM.md).

## Hints

-   The rule dictating that ABAP SQL is to be used for as long as possible applies in particular to [AMDP](ABENAMDP.md) too. It is not a good idea to swap out SQL statements to database procedures if these could be implemented using ABAP SQL or [ABAP CDS](ABENCDS_GLOSRY.md) too. In this case, no performance gains can be expected since the ABAP SQL statements are updated to Native SQL by the database interface in exactly the same way as they would be written in the database procedure. Using AMDP is a good idea only if HANA-specific properties can be exploited by procedure calls or if repeated transports of large amounts of data between the database and the AS ABAP can be bypassed.
...
See the executable example [AMDP, Comparison with ABAP SQL](ABENAMDP_VS_ABAP_SQL_CS_ABEXA.md). Database access not programmed well using ABAP SQL can often be optimized by improved use of ABAP SQL, making it unnecessary to use AMDP in these cases.

See the executable example [Currency Conversion with SQLScript](ABENSQL_SCRIPT_CURR_CONV_ABEXA.md). In this case, a specific built-in function of HANA SQL is used that was not generally available in ABAP SQL. Meanwhile, the function [`currency_conversion`](ABENSQL_CURR_UNIT_CONV_FUNC.md) is available in ABAP SQL and can replace the usage of AMDP.
