...
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENUSE_SHARED_MEMORY_GUIDL.html"
...
-   To buffer data from database tables implicitly using [table buffering](ABENTABLE_BUFFERING_GLOSRY.md), which can be determined when defining the tables in ABAP Dictionary
...
-   (Data) objects can be used in the shared objects memory just like objects in the [internal session](ABENINTERNAL_SESSION_GLOSRY.md). Technically, the shared objects memory can be considered an extension of the internal session during the time the memory is bound to it.
