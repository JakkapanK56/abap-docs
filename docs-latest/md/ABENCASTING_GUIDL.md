...
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCASTING_GUIDL.html"
...
-   Explicit casting is possible by using the `CASTING` addition with the `ASSIGN` statement and by using the `ASSIGNING` addition in statements for processing internal tables. Assignments between reference variables allow [upcasts](ABENUP_CAST_GLOSRY.md) and [downcasts](ABENDOWN_CAST_GLOSRY.md). Obsolete explicit casting is also possible for formal parameters and field symbols, if the `STRUCTURE` addition is used.
