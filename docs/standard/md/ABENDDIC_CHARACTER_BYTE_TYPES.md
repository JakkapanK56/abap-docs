...
description: |
  ABENDDIC_CHARACTER_BYTE_TYPES - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "types"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENDDIC_CHARACTER_BYTE_TYPES.htm"
...
When a field with character-like dictionary type is used from a [dynpro](ABENDYNPRO_GLOSRY.html), lowercase letters are transformed to uppercase letters by default. This behavior can be overridden in Screen Painter. Furthermore, [lowercase letters](ABENDDIC_DOMAINS_SEMA.html) can be preserved for data elements in the semantic properties of a domain.
...
-   If DDIC structures, DDIC database tables, or DDIC views that have strings as components are used for typings in ABAP, it must be remembered that these objects are [deep structures](ABENDEEP_STRUCTURE_GLOSRY.html). This means, for example, that they cannot be used in the [`TABLES`](ABAPTABLES.html) statement (obsolete for database accesses) or for typing the obsolete [table parameters](ABENTABLE_PARAMETER_GLOSRY.html) of procedures.
...
-   In the [`SELECT` list](ABAPSELECT_LIST.html) when the addition [`FOR ALL ENTRIES`](ABENWHERE_ALL_ENTRIES.html) is used. In the [strict mode from ABAP release 7.40, SP05](ABENABAP_SQL_STRICTMODE_740_SP05.html), this produces a syntax error. Otherwise, the extended program check produces a syntax warning that can be hidden by a pragma.
