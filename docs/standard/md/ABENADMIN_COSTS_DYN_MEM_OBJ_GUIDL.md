---
title: "ABENADMIN_COSTS_DYN_MEM_OBJ_GUIDL"
description: |
  ABENADMIN_COSTS_DYN_MEM_OBJ_GUIDL - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "ui"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENADMIN_COSTS_DYN_MEM_OBJ_GUIDL.htm"
abapFile: "ABENADMIN_COSTS_DYN_MEM_OBJ_GUIDL.html"
...
With each additional secondary table key, the [memory](ABENSECONDARY_KEY_GUIDL.html) requirement increases by the memory needed for secondary key management (secondary index or secondary hash management).
...
At a low dataset level, class wrapping can be considered as an alternative to internal tables. This is because the extra costs for objects are comparatively low and objects can be completely deleted from the memory (unlike dynamic data objects). This is an exception to the rule that [dynamic data objects](ABENUSE_DYN_DATA_OBJECT_GUIDL.html) should be used wherever possible.

Besides the ratio of administration data to application data, the ratio of the [memory allocated](ABENINITIAL_MEMORY_REQU_GUIDL.html) for application data and the memory actually used is also interesting for internal tables.

The executable example [`CL_DEMO_MEMORY_USAGE`](ABENMEMORY_USAGE_ABEXA.html) demonstrates the administration costs of deep components with low data content.
...
-   18 bytes per table line on average for the hash management, provided that the table is not accessed using the `DELETE` or `SORT` statements. If one of these access types occurs, an average of 30 bytes is required per table line.
