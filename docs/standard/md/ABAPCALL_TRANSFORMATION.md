...
description: |
  ABAPCALL_TRANSFORMATION - Standard ABAP language reference documentation
library: "standard"
...
[1a. `... SOURCE \{XML src_xml\}|\{\{bn1 = e1 bn2 = e2 ...\}|(stab)\}`](#ABAP_ADDITION_1a@3a@)

[1b. `... RESULT \{XML rslt_xml\}|\{\{bn1 = f1 bn2 = f2 ...\}|(rtab)\}`](#ABAP_ADDITION_1b@3b@)
...
-   In the case of XSLT, a [DOM](ABENDOM_GLOSRY.html) is always needed as a source. This DOM is constructed and processed using the [iXML Library for ABAP Cloud](ABENABAP_IXML_LIB_CLOUD.html) or the [iXML Library Classic](ABENABAP_IXML_LIB.html). In the case of the [identity transformation](ABENID_TRAFO_GLOSRY.html)\\ `ID`, the [iXML Library for ABAP Cloud](ABENABAP_IXML_LIB_CLOUD.html) or the [iXML Library Classic](ABENABAP_IXML_LIB.html) is used only if the corresponding objects are specified as source or target.
-   ST is processed serially, using the [sXML Library](ABENABAP_SXML_LIB.html).
...
-   As an interface reference variable for objects in the [iXML Library for ABAP Cloud](ABENABAP_IXML_LIB_CLOUD.html) or the [iXML Library Classic](ABENABAP_IXML_LIB.html) (XSLT only):

-   Interface reference variable with the type `IF_IXML_ISTREAM_CORE` or `IF_IXML_ISTREAM` that points to an [input stream](ABENABAP_IXML_LIB_C_INPUT_OUTPUT.html) that reads XML data.
...
-   As a reference variable of the type `IF_SXML_READER` or `CL_SXML_READER` for [XML Reader](ABENXML_READER_GLOSRY.html) in the [sXML Library](ABENABAP_SXML_LIB.html). The transformation moves the reader by one node and processes the entire matching subtree. If the transformation is successful, the reader is located at the end of the processed data. Any valid reader can be specified:
...
-   The interface `IF_SXML_READER` and the class `CL_SXML_READER` are part of the API of the [sXML Library](ABENABAP_SXML_LIB.html), whose objects can be accessed using `CALL TRANSFORMATION`.
...
-   The interfaces `IF_IXML_ISTREAM` and `IF_IXML_NODE` are part of the API of the [iXML Library Classic](ABENABAP_IXML_LIB.html), whose objects can be accessed using `CALL TRANSFORMATION`.
...
-   For an overview of all possible XML sources, see [XML Sources of Transformations](ABENCALL_TRAFO_SOURCES_ABEXA.html).
...
-   Interface reference variable with the type `IF_IXML_OSTREAM_CORE` or `IF_IXML_OSTREAM` that points to an [output stream](ABENABAP_IXML_LIB_C_INPUT_OUTPUT.html) that writes XML data.
...
-   As a reference variable with the type `IF_SXML_WRITER` or `CL_SXML_WRITER` for [XML](ABENXML_WRITER_GLOSRY.html) writers in the [sXML](ABENABAP_SXML_LIB.html) Library. The result of the transformation is added to the current element of the writer as a subtree. Any valid writer can be specified:
...
-   The interface `IF_SXML_WRITER` and the class `CL_SXML_WRITER` are part of the API of the [sXML Library](ABENABAP_SXML_LIB.html), whose objects can be accessed using `CALL TRANSFORMATION`.
...
-   The interfaces `IF_IXML_OSTREAM` and `IF_IXML_DOCUMENT` are part of the API of the [iXML Library Classic](ABENABAP_IXML_LIB.html), whose objects can be accessed using `CALL TRANSFORMATION`.
...
-   For an overview of all possible XML sources, see [XML Sources of Transformations](ABENCALL_TRAFO_RESULTS_ABEXA.html).
...
-   For transformations of XML data to the DOM representation in the iXML Library Classic, see [iXML Library, DOM Representation](ABENIXML_DOM_ABEXA.html).
...
-   The special addition [`OBJECTS`](ABAPCALL_TRANSFORMATION_OBJECTS.html) can be used to pass object references, but this addition is replaced by `PARAMETERS`.
...
-   Methods of the [iXML Library Classic](ABENABAP_IXML_LIB.html) or [sXML Library](ABENABAP_SXML_LIB.html) are involved that are implemented as [kernel methods](ABENKERNEL_METHOD_GLOSRY.html).
