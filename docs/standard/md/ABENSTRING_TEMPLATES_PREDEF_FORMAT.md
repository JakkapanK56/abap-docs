...
description: |
  ABENSTRING_TEMPLATES_PREDEF_FORMAT - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENSTRING_TEMPLATES_PREDEF_FORMAT.htm"
...
Unlike in [`WRITE TO`](ABENWRITE_FORMATS.html), no formatting is applied to the data types `d` and `t` and no separators are inserted if no [formatting options](ABAPCOMPUTE_STRING_FORMAT_OPTIONS.html) are specified explicitly.
