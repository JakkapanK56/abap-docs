...
description: |
  ABAPASSIGN_DYNAMIC_COMPONENTS - Standard ABAP language reference documentation
library: "standard"
...
-   For the latter see this [executable example](ABENASSIGN_COMPONENT_ABEXA.html).

-   The first implementation does not use [RTTI](ABENRUN_TIME_TYPE_IDENTIFIC_GLOSRY.html). The statement [`DESCRIBE FIELD`](ABAPDESCRIBE_FIELD.html) is used to check whether the passed data object is a structure. The components are then assigned one after another to the field symbol in a `DO` loop.
-   The second implementation uses [RTTI](ABENRUN_TIME_TYPE_IDENTIFIC_GLOSRY.html). A [downcast](ABENDOWN_CAST_GLOSRY.html) of the type description object to the class `CL_ABAP_STRUCTDESCR` for the passed data object ensures that the object is a structure. A loop across the component table `COMPONENTS` assigns the components to the field symbol via their names.
