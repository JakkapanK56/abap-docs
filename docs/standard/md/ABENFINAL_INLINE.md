...
description: |
  ABENFINAL_INLINE - Standard ABAP language reference documentation
library: "standard"
...
The declaration operator `FINAL` can be used in the same [declaration positions](ABENDECLARATION_POSITION_GLOSRY.html) as the operator `DATA` with one exception: `FINAL` cannot be used behind [`OPEN CURSOR`](ABAPOPEN_CURSOR.html) because usually there is always a second write access with [`CLOSE CURSOR`](ABAPCLOSE_CURSOR.html) in the same context.
