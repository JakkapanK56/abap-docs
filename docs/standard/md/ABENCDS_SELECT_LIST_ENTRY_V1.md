...
description: |
  ABENCDS_SELECT_LIST_ENTRY_V1 - Standard ABAP language reference documentation
library: "standard"
...
Specifies all elements of an [extension](ABENDDIC_ENHANCEMENTS.html) in the extension concept for dictionary objects. If specified, `$EXTENSION.*` inserts (in this place) all fields of all [customizing includes](ABENDDIC_CUSTOMIZING_INCLUDES.html), [append structures](ABENDDIC_APPEND_STRUCTURES.html), and [DDIC append views](ABENDDIC_APPEND_VIEWS.html) that exist in the current system for the DDIC database tables or [DDIC views](ABENDDIC_VIEWS.html) and which occur as a data source [`data_source`](ABENCDS_DATA_SOURCE_V1.html) in the current CDS view. `$EXTENSION.*` is ignored for CDS views used as a data source and extended using [`EXTEND VIEW`](ABENCDS_EXTEND_VIEW.html).
...
-   With respect to the element annotations, publications of a CDS association [`association`](ABENCDS_SELECT_LIST_ASSOCIATION_V1.html) are handled like any regular element. When a CDS association defined in a different CDS view is exposed, the annotations of preceding publications are inherited when [annotations are evaluated](ABENCDS_ANNOTATIONS_ANALYSIS.html) using the class `CL_DD_DDL_ANNOTATION_SERVICE`.
...
-   In the syntax check in ABAP SQL, the key elements used depend on whether the name of the CDS entity or the name of the CDS-managed DDIC view (obsolete) is specified (with the latter being [obsolete](ABENABAP_SQL_CDS_OBSOLETE.html)).
