---
title: "ABENTABLE_OUTPUT_GUIDL"
description: |
  ABENTABLE_OUTPUT_GUIDL - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "data-structures"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENTABLE_OUTPUT_GUIDL.htm"
abapFile: "ABENTABLE_OUTPUT_GUIDL.html"
keywords: ["select", "insert", "loop", "do", "if", "case", "data", "internal-table", "field-symbol", "ABENTABLE", "OUTPUT", "GUIDL"]
---

Internal tables can be read by accessing individual lines using `READ TABLE` or [table expressions](ABENTABLE_EXPRESSION_GLOSRY.html), or sequentially using `LOOP AT`. In both cases, the following output behavior can be defined by using the statements with the following additions:

In the case of [table expressions](ABENTABLE_EXPRESSIONS.html), the output behavior is controlled by the [category of the result](ABENTABLE_EXP_RESULT.html).
...
For table expressions, the information here applies to the selection of the appropriate [result](ABENTABLE_EXP_RESULT.html).
...
-   The addition `ASSIGNING` assigns the read line to a [field symbol](ABENDYN_ACCESS_DATA_OBJ_GUIDL.html), which enables the line to be addressed directly.
-   The `REFERENCE INTO` addition sets a [data reference](ABENDYN_ACCESS_DATA_OBJ_GUIDL.html) to the read line.
...
-   If the content of the read line is to be modified, the addition `ASSIGNING` or (in the case of table expressions) the appropriate [result](ABENTABLE_EXP_RESULT.html) should usually be used. This allows direct access to the line using the value semantics and removes the need for a `MODIFY` operation later on.
-   If a reference to the read line is required that can be processed using reference semantics, the addition `REFERENCE INTO` or (in the case of table expressions) the appropriate [result](ABENTABLE_EXP_RESULT.html) is to be used.
-   If the content of the read line is not to be modified, any of these procedures can be used. The line type of the table is significant for performance. If the table line is wide or contains deep components (for example, strings or other tables), reads are usually faster if `ASSIGNING` or `REFERENCE INTO` is used instead of `INTO`. The [way they are used](ABENDYN_ACCESS_DATA_OBJ_GUIDL.html) is the determining factor for selecting which of the two should be used.
