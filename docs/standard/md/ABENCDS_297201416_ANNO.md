---
title: "ABENCDS_297201416_ANNO"
description: |
  ABENCDS_297201416_ANNO - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCDS_297201416_ANNO.htm"
abapFile: "ABENCDS_297201416_ANNO.html"
keywords: ["do", "if", "class", "data", "ABENCDS", "297201416", "ANNO"]
---
...
If a CDS view returns enormous amount of data records, to avoid out-of-memory issues in HANA, it is necessary to split a full extraction request into several disjoint data packages. Annotation *partitionBy* defines a list of view elements that can be used to build those packages and data records of THE same partitionBy elements are read within the same database transaction.

If this annotation is not specified, a full extraction will read all data with one database cursor.

**Kind** [Framework-specific annotation](ABENFRMWRK_ANNOTATION_GLOSRY.html)\\ **Syntax** `Analytics.dataExtraction.partitionBy[ ]`\\ **Scope** `#HIERARCHY`, `#TABLE_FUNCTION`, `#VIEW`\\ **Type** `ElementRef`\\ **Enums** `-`\\ **Default** `-`\\ **Text** `-`\\ **MDE** `-`\\ **API** `NOT_RELEASED` abenabap.html abencds.html abencds\_annotations.html abencds\_annotations\_sap.html abencds\_annotations\_ktd\_docu.html abencds\_frmwrk\_annos.html abencds\_analytics\_f.html