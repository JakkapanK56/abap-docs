...
description: |
  ABENABAP_SXML_LIB - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENABAP_SXML_LIB.htm"
...
-   For examples, see the corresponding executable [programs in the example library](ABENABAP_SXML_LIB_ABEXAS.html) and the package `SXML_DEMO`.
-   If access to all nodes of an XML document in the memory is not required and no DTD is needed, the sXML Library is an alternative to the iXML Library with better performance. It also supports more XML formats and enables the handling of [JSON](ABENJSON_GLOSRY.html) in ABAP programs.
