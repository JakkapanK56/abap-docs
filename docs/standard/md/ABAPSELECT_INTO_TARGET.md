...
description: |
  ABAPSELECT_INTO_TARGET - Standard ABAP language reference documentation
library: "standard"
...
As in the previous example, but after [`FETCH`](ABAPFETCH.html) and with a dynamically specified `SELECT` list and the `FROM` clause after [`OPEN CURSOR`](ABAPOPEN_CURSOR.html). In this case, it is not possible to make a direct inline declaration of the target area in the `INTO` clause. It is, however, possible to use the addition `NEW`.
...
The columns of the result set defined in the `SELECT` list must have unique names. If multiple [`FETCH`](ABAPFETCH.html) statements access a database cursor opened using [`OPEN CURSOR`](ABAPOPEN_CURSOR.html), `NEW` and an inline declaration can be used only if this is also the case in the first of these `FETCH` statements in the corresponding operand position.
...
-   The same applies to inline declarations after [`FETCH`](ABAPFETCH.html).
...
-   After [`FETCH`](ABAPFETCH.html), an inline declaration can only be made together with the addition `NEW`.
...
-   If the obsolete addition [`CLIENT SPECIFIED`](ABAPSELECT_CLIENT_OBSOLETE.html) is used to access a client-dependent [CDS entity](ABENCDS_ENTITY_GLOSRY.html), a name must be specified for the client column at the same time.
...
-   Therefore, this kind of inline declaration is only [suitable](ABENSELECT_TABLE_TYPE_GUIDL.html) for tables, which are mainly accessed by an index. If key accesses are important, the internal table should not be declared inline but with a declaration statement and with an appropriate key.
...
-   After [`FETCH`](ABAPFETCH.html).

-   The addition `NEW` can only be specified after [`INTO`](ABAPINTO_CLAUSE.html) and not after [`APPENDING`](ABAPINTO_CLAUSE.html).
-   If multiple [`FETCH`](ABAPFETCH.html) statements access a database cursor opened using [`OPEN CURSOR`](ABAPOPEN_CURSOR.html), a data reference variable used after `NEW` can be typed generically only if the first of these `FETCH` statements has the addition `NEW` with a generically typed data reference variable in the appropriate operand position.
...
-   If the addition `NEW` is used, the syntax check is performed in [strict mode from ABAP release 7.77](ABENABAP_SQL_STRICTMODE_777.html).

-   If the data type of the new anonymous data object is known statically, `dref` is also typed with this type.
-   If the data type of the new anonymous data object cannot be known until runtime, `dref` is typed with the generic type [`data`](ABENBUILT_IN_TYPES_GENERIC.html). This is the case when the structure of the result set is not known statically due to the use of dynamic tokens and in inline declarations after [`FETCH`](ABAPFETCH.html).
...
-   The addition `NEW` can also be used to make an inline declaration after [`FETCH`](ABAPFETCH.html). The data reference variable that points to the target area created as an anonymous data object is declared, however, instead of the direct target area.
