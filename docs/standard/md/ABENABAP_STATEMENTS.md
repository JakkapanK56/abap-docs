---
title: "Specific ABAP words, operands, and operators can be combined to"
description: |
  which can be specified at certain operand positions. In restricted ABAP language versions(ABENRESTRICTED_VERSION_GLOSRY.html) not all language elements can be used. The topic Language Elements in ABAP Versions(ABENRESTRICTED_ABAP_ELEMENTS.html) provides an overview of the ABAP language elements
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENABAP_STATEMENTS.htm"
abapFile: "ABENABAP_STATEMENTS.html"
keywords: ["delete", "do", "if", "case", "method", "class", "data", "ABENABAP", "STATEMENTS"]
---

[ABAP statements](ABENABAP_STATEMENTS_OVERVIEW.html) consist of the following [tokens](ABENTOKEN_GLOSRY.html) and always end with a period (`.`):

Specific ABAP words, operands, and operators can be combined to

which can be specified at certain operand positions.

In [restricted ABAP language versions](ABENRESTRICTED_VERSION_GLOSRY.html) not all language elements can be used. The topic [Language Elements in ABAP Versions](ABENRESTRICTED_ABAP_ELEMENTS.html) provides an overview of the ABAP language elements that are allowed in restricted language versions compared to [Standard ABAP](ABENSTANDARD_ABAP_GLOSRY.html).
...
-   Some expressions use [obsolete syntax forms](ABENTOKENIZATION_OBSOLETE.html) that can still occur outside classes, where the separators between the tokens can be omitted.
-   The absolute maximum length of an ABAP statement is 128 x 1024 characters = 131072 characters. However, the actual maximum number of characters in a statement also depends on the number of tokens and the use of literals. In general, a statement can consist of almost 130000 characters. In [chained statements](ABENCHAINED_STATEMENT_GLOSRY.html), however, there are only a maximum of 28400 characters per statement.
