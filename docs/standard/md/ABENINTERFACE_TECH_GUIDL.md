---
title: "ABENINTERFACE_TECH_GUIDL"
description: |
  ABENINTERFACE_TECH_GUIDL - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "oop"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENINTERFACE_TECH_GUIDL.htm"
abapFile: "ABENINTERFACE_TECH_GUIDL.html"
keywords: ["select", "if", "case", "class", "data", "ABENINTERFACE", "TECH", "GUIDL"]
---
...
Web Dynpro ABAP and SAPUI5 are SAP's standard `UI` technologies for developing modern Web applications in ABAP. The MVC approach automatically ensures a separation of presentation logic and application logic, based on the [SoC principle](ABENSEPARATION_CONCERNS_GUIDL.html).
...
-   **Web Dynpro ABAP**
-   Web Dynpro ABAP is a technology for creating platform-independent, Web-based interfaces. The architecture of Web Dynpro is based on the Model View Controller approach (MVC) for [SoC](ABENSEPARATION_CONCERNS_GUIDL.html). The three components of the model view controller are the data model, which describes the application, the presentation view, and the program controller for responding to user actions. Web Dynpro applications are created using Web Dynpro Explorer in ABAP Workbench and appear in AS ABAP using generated ABAP Objects classes.
-   **SAPUI5**
-   SAPUI5 is a tool set for developing user interfaces for Web applications in HTML5. SAPUI5 supports the Model View Controller (MVC) approach and based on the user interface elements delivered in JavaScript and CSS libraries. The data connection to the application server is established using the data formats [XML](ABENXML_GLOSRY.html), [JSON](ABENJSON_GLOSRY.html) or OData. User interfaces developed with SAPUI5 can be connected to SAP and non-SAP platforms. In particular, SAPUI5 user interfaces can be developed for ABAP applications of [AS ABAP](ABENAS_ABAP_GLOSRY.html). The connection is established using [ICF](ABENICF_GLOSRY.html) and [APC](ABENAPC_GLOSRY.html).

-   In general, the UI technology of an application that is strictly modeled on the [separation of concerns](ABENSEPARATION_CONCERNS_GUIDL.html) can be replaced without any problems (see example below).
