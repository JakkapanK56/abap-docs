...
description: |
  ABENEQUALS_OPERATOR - Standard ABAP language reference documentation
library: "standard"
...
-   If `lhs` is a data object, it can be prefixed with the obsolete keyword [`COMPUTE`](ABAPCOMPUTE.html), which has no effect however and should therefore be omitted.
-   Another obsolete form of the assignment is the statement [`MOVE rhs TO lhs`](ABAPMOVE_OBS.html), where the left side is on the right side and does not include all options of the assignment operator `=`.
