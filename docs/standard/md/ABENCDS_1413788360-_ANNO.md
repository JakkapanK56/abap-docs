...
title: "SUM"
description: |
  MIN MAX AVG COUNT_DISTINCT. They correspond to the SQL aggregation functions. Kind Framework-specific annotation(ABENFRMWRK_ANNOTATION_GLOSRY.html) Syntax `Consumption.derivation.resultAggregation` Scope `#ELEMENT`, `#PARAMETER` Type `String(30)` Enums `#AVG, #COUNT_DI
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCDS_1413788360-_ANNO.htm"
abapFile: "ABENCDS_1413788360-_ANNO.html"
keywords: ["select", "do", "if", "data", "ABENCDS", "1413788360", "ANNO"]
---
...
If resultAggregation is used, the select on the lookupEntity is performed with the aggregation "resultAggregation (resultElement )". If resultAggregationHigh is used, the select is performed with aggregation "resultAggregationHigh (resultElementHigh )". If resultElement and resultElementHigh are set, the aggregation must be set for either both or no fields. The available aggregations are:
...
They correspond to the SQL aggregation functions.

**Kind** [Framework-specific annotation](ABENFRMWRK_ANNOTATION_GLOSRY.html)\\ **Syntax** `Consumption.derivation.resultAggregation`\\ **Scope** `#ELEMENT`, `#PARAMETER`\\ **Type** `String(30)`\\ **Enums** `#AVG, #COUNT_DISTINCT, #MAX, #MIN, #SUM`\\ **Default** `-`\\ **Text** `-`\\ **MDE** `X`\\ **API** `RELEASED_FOR_KEY_USER_APPS, RELEASED_FOR_SAP_CLOUD_PLATFORM` abenabap.html abencds.html abencds\_annotations.html abencds\_annotations\_sap.html abencds\_annotations\_ktd\_docu.html abencds\_frmwrk\_annos.html abencds\_consumption\_f.html