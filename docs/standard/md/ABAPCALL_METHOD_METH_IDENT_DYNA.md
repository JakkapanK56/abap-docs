...
description: |
  ABAPCALL_METHOD_METH_IDENT_DYNA - Standard ABAP language reference documentation
library: "standard"
...
keywords: ["do", "if", "case", "method", "class", "data", "types", "ABAPCALL", "METHOD", "METH", "IDENT", "DYNA"]
---
...
-   The [pseudo reference](ABENPSEUDO_REFERENCE_GLOSRY.html)\\ `super` cannot be used in a dynamic method call.
...
-   External calls of local class methods is [critical](ABENPROGRAM_GROUPS.html), especially for executable programs, module pools, and subroutine pools, since it is not usually possible to statically determine to which program group such programs are assigned.
-   If, in `class_name`, a class of another program is specified using an [absolute type name](ABENABSOLUTE_TYPENAME_GLOSRY.html), this program is loaded, depending on the program type, into a new [additional program group](ABENADDITIONAL_PROG_GROUP_GLOSRY.html) or into the current [program group](ABENPROGRAM_GROUP_GLOSRY.html), if it has not been loaded already. If required, the [program constructor](ABENPROGRAM_CONSTRUCTOR_GLOSRY.html) is also executed.
