...
title: "ABENABAP_UNIT"
description: |
  ABENABAP_UNIT - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENABAP_UNIT.htm"
abapFile: "ABENABAP_UNIT.html"
keywords: ["do", "if", "method", "class", "data", "ABENABAP", "UNIT"]
---

ABAP Unit is a test framework for [unit tests](ABENUNIT_TEST_GLOSRY.html). ABAP Unit can be used to:

Single tests can be started directly in the tool for the corresponding program. More comprehensive test runs are executed from the ABAP Unit Browser in the ABAP Workbench or by using the ABAP Test Cockpit. ABAP Unit is closely linked with the [Coverage Analyzer](ABENCOVERAGE_ANALYZER_GLOSRY.html), enabling test coverage to be measured as well.
...
-   During a unit test, it is not allowed to exit the production code that is called from a test method with the statements [`LEAVE PROGRAM`](ABAPLEAVE_PROGRAM.html)\\ [`LEAVE TO TRANSACTION`](ABAPLEAVE_TO_TRANSACTION.html) and [`SUBMIT`](ABAPSUBMIT.html) without `AND RETURN`.
-   In test classes, a [replacement service for ABAP SQL](ABENCL_OSQL_REPLACE.html) enables access to data sources by ABAP SQL statements to be redirected to other database objects.
