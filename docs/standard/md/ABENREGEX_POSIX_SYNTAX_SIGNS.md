...
description: |
  ABENREGEX_POSIX_SYNTAX_SIGNS - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENREGEX_POSIX_SYNTAX_SIGNS.htm"
...
The following table shows some results of a [matching](ABENREGEX_MTCH.html) that is case-sensitive.

The regular expression `AB` is a [concatenation](ABENREGEX_POSIX_SYNTAX_OPERATORS.html) of two expressions for single characters.
...
The following table shows some results of a [matching](ABENREGEX_MTCH.html) that is not case-sensitive.

The regular expression `..` is a [concatenation](ABENREGEX_POSIX_SYNTAX_OPERATORS.html) of two expressions for single characters.
...
The regular expression `[AB][CD]` is a [concatenation](ABENREGEX_POSIX_SYNTAX_OPERATORS.html) of two expressions for single characters.
...
The regular expression `[^A][^B]` is a [concatenation](ABENREGEX_POSIX_SYNTAX_OPERATORS.html) of two expressions for single characters.
...
**Pattern** `text` **match** **Pattern** **Text** **match** **Pattern** **Text** **match** **Pattern** **Text** **match** **Pattern** **Text** **match** **Pattern** **Text** **match** **Character Set** **Abbr.** **Meaning** **Pattern** **Text** **match** *A* *A* X *A* `a` - `\\.` `.` X *A* `AB` - `AB` `AB` X `.` *A* X `\\C` `a` X `.` `AB` - `..` `AB` X `[ABC]` `B` X `[ABC]` `ABC` - `[AB][CD]` `AD` X `[\\d]` `9` X `[^ABC]` `B` - `[^ABC]` `Y` X `[^A][^B]` `BA` X `[A^B]` `^` X `[A-Za-z0-9]` `B` X `[A-Za-z0-9]` `5` X `[A-Za-z0-9]` `#` - `[A-Za-z0-9]` `-` - `[A-Za-z0-9-]` `-` X `[[:alnum:]]` `a` X `[[:alnum:]]` `;` - `[[:alpha:]]` `1` - `[[:punct:]]` `.` X `[[:lower:]]` `â` X `[[:upper:]]` `Ä` X `[[:digit:][:alpha:]]+` `a1b2c3` X `[[:digit:]]` `\\d` Placeholder for a digit `[^[:digit:]]` `\\D` Placeholder for a non-digit `[[:lower:]]` `\\l` Placeholder for a lowercase letter `[^[:lower:]]` `\\L` Placeholder for a character that is not a lowercase letter `[[:space:]]` `\\s` Placeholder for a blank character `[^[:space:]]` `\\S` Placeholder for a non-blank character `[[:upper:]]` `\\u` Placeholder for an uppercase letter `[^[:upper:]]` `\\U` Placeholder for a character that is not an uppercase letter `[[:word:]]` `\\w` Placeholder for an alphanumeric character plus underscore \_ `[^[:word:]]` `\\W` Placeholder for an non-alphanumeric character without underscore \_ `\\d` `4` X `\\D` `;` X `\\l` `u` X `\\l` `U` - `\\L` `S` X `\\s` X `\\S` `#` X `\\u` `U` X `\\U` `.` X `\\w` *A* X `\\w` `8` X `\\W` `:` X `\\W` `_` - abenabap.html abenabap\_reference.html abenabap\_obsolete.html abendata\_internal\_obsolete.html abencharacter\_string\_obsolete.html abenregular\_expressions\_obsolete.html abenregex\_posix\_syntax.html