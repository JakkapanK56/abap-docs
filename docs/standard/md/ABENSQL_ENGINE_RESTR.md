...
description: |
  ABENSQL_ENGINE_RESTR - Standard ABAP language reference documentation
library: "standard"
...
keywords: ["select", "update", "if", "case", "class", "data", "types", "internal-table", "ABENSQL", "ENGINE", "RESTR"]
---
...
-   Access to a table or view for which a [replacement object](ABENDDIC_REPLACEMENT_OBJECTS.html) is defined.
-   Use of a [secondary connection](ABENSECONDARY_DB_CONNECTION_GLOSRY.html) specified explicitly using the addition [`CONNECTION`](ABAPSELECT_CONNECTION.html). If specified, a [standard connection](ABENSTANDARD_DB_CONNECTION_GLOSRY.html) or a [service connection](ABENSERVICE_CONNECTION_GLOSRY.html) can be processed by the ABAP SQL in-memory engine.
-   Use of the class [`CL_OSQL_REPLACE`](ABENCL_OSQL_REPLACE.html) during a unit test.
...
-   The obsolete addition [`CLIENT SPECIFIED`](ABAPSELECT_CLIENT_OBSOLETE.html) if the [client column](ABENCLIENT_COLUMN_GLOSRY.html) is not specified in a `WHERE` condition.
