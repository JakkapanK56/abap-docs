...
description: |
  ABAPLOOP_AT_ITAB_GROUP_BY - Standard ABAP language reference documentation
library: "standard"
...
-   In this [example](ABENINT_TABLE_AT_UNSORTED_ABEXA.html), `GROUP BY` is used to replace group level processing.
-   For groupings using `GROUP BY` to replace self-programmed group loops see the [example](ABENLOOP_GROUP_BY_EXPLICIT_ABEXA.html)).
