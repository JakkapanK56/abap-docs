---
title: "ABENABAP_IXML_LIB_DOM_ITERAT_READ"
description: |
  ABENABAP_IXML_LIB_DOM_ITERAT_READ - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENABAP_IXML_LIB_DOM_ITERAT_READ.htm"
abapFile: "ABENABAP_IXML_LIB_DOM_ITERAT_READ.html"
keywords: ["select", "do", "if", "case", "method", "data", "ABENABAP", "IXML", "LIB", "DOM", "ITERAT", "READ"]
---

The section [Direct Read](ABENABAP_IXML_LIB_DOM_DIRECT_READ.html) demonstrates how to access the DOM nodes directly using the following objects:
...
[Iterator for Nodes](ABENIXML_NODE_ITERATOR_ABEXA.html)

An iterator can be created for all of the lists under [Direct Read](ABENABAP_IXML_LIB_DOM_DIRECT_READ.html):
...
-   Unlike a node [direct read](ABENABAP_IXML_LIB_DOM_DIRECT_READ.html), which can be restricted to the elements of the represented XML data, an iterator captures all nodes in an XML document, including nodes that only contain structural information. The method `GET_TYPE` of interface `IF_IXML_NODE` should be used to read the node type and compare it to constants of this interface. [Filter](ABENABAP_IXML_LIB_DOM_FILTER_READ.html) or [Downcasts](ABENIXML_DOWN_CASTS.html) provide further options for only selecting specified nodes.
...
-   [Iterator for Node List](ABENIXML_NODE_LIST_ITERATOR_ABEXA.html)
-   [Iterator for Element List](ABENIXML_NAME_LIST_ITERATOR_ABEXA.html)
-   [Iterator for Attributes](ABENIXML_ATTRIBUTES_ITERATOR_ABEXA.html)
