...
description: |
  ABENABAP_SQL_EXPR_LOGEXP - Standard ABAP language reference documentation
library: "standard"
...
-   A client column can be used as an operand only if [implicit client handling](ABENABAP_SQL_CLIENT_HANDLING.html) is disabled using the addition `CLIENT SPECIFIED` for the [query](ABAPSELECT_CLIENT_OBSOLETE.html) or the [write](ABAPUD_CLIENT_OBSOLETE.html). This is not possible for [filter conditions](ABENABAP_SQL_PATH_FILTER.html).
...
-   SQL expressions as operands on the right side of an SQL condition enforce the [strict mode from ABAP release 7.90](ABENABAP_SQL_STRICTMODE_790.html).
