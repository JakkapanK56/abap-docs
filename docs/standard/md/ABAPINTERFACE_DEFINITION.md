...
description: |
  ABAPINTERFACE_DEFINITION - Standard ABAP language reference documentation
library: "standard"
...
The addition `PUBLIC` makes the interface `intf` a [global interface](ABENGLOBAL_INTERFACE_GLOSRY.html) of the class library. The addition `PUBLIC` can only be used for the global interface of an [interface pool](ABENINTERFACE_POOL_GLOSRY.html) and is created by the [Class Builder](ABENCLASS_BUILDER_GLOSRY.html) when a global interface is created.
