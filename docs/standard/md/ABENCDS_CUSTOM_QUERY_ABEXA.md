---
title: "ABENCDS_CUSTOM_QUERY_ABEXA"
description: |
  ABENCDS_CUSTOM_QUERY_ABEXA - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCDS_CUSTOM_QUERY_ABEXA.htm"
...
The example accesses the CDS custom entity `DEMO_CDS_CUSTOM_ENTITY` and fills it with data using the [service binding](ABENSERVICE_BINDING_GLOSRY.html)\\ `DEMO_SB_CUSTOM_QUERY`, which binds the service definition `DEMO_SD_CUSTOM_ENTITY` to an OData node in [ICF](ABENICF_GLOSRY.html). This is done using the demonstration class `CL_DEMO_BS_CONSUMPTION`, which accesses the OData service of the service binding by using an [ICF](ABENICF_GLOSRY.html) client.
