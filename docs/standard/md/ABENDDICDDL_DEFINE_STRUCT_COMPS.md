...
description: |
  ABENDDICDDL_DEFINE_STRUCT_COMPS - Standard ABAP language reference documentation
library: "standard"
...
-   The additions `KEY` and `NOT NULL` can be used to define the component as a [key field](ABENDDIC_STRUCTURES_SEMA.html) and to set a [flag for initial values](ABENDDIC_STRUCTURES_SEMA.html). The addition `KEY` has no effect when the structure is included in DDIC database tables. It must be used for structure components that are used as lock parameters of lock objects. The addition `NOT NULL` can be respected for structures that are included in DDIC database tables.
