---
title: "ABENCDS_487557726-_ANNO"
description: |
  ABENCDS_487557726-_ANNO - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCDS_487557726-_ANNO.htm"
abapFile: "ABENCDS_487557726-_ANNO.html"
keywords: ["do", "if", "case", "class", "data", "ABENCDS", "487557726", "ANNO"]
---
...
This annotation enables extraction of special fields in an Entity which cannot be normally extracted because of the below constraints:

This annotation has no effect in the CDS Views annotated by 'Analytics.DataExtraction.changeDataCapture'.

-   They are neither key fields nor fields which are relevant for Texts (for example annotated with @Semantics.text:true or @Semantics.Language:true ) in Text View (@ObjectModel.dataCategory: #TEXT )
-   They are neither the fields in Hierarchy View (@ObjectModel.dataCategory: #HIERARCHY ) which are annotated by foreign key associations (@ObjectModel.foreignKey.association ) nor having special annotation @Semantics.signReversalIndicator.
-   They are annotated by @Analytics.Hidden in case of Dimension (@Analytics.dataCategory: #DIMENSION ) and Cube View (@Analytics.dataCategory: #CUBE )

-   They are neither key fields nor fields which are relevant for Texts (for example annotated with @Semantics.text:true or @Semantics.Language:true ) in Text View (@ObjectModel.dataCategory: #TEXT )
-   They are neither the fields in Hierarchy View (@ObjectModel.dataCategory: #HIERARCHY ) which are annotated by foreign key associations (@ObjectModel.foreignKey.association ) nor having special annotation @Semantics.signReversalIndicator.
-   They are annotated by @Analytics.Hidden in case of Dimension (@Analytics.dataCategory: #DIMENSION ) and Cube View (@Analytics.dataCategory: #CUBE )

**Kind** [Framework-specific annotation](ABENFRMWRK_ANNOTATION_GLOSRY.html)\\ **Syntax** `Analytics.dataExtraction.relevant`\\ **Scope** `#ELEMENT`\\ **Type** `Boolean`\\ **Enums** `-`\\ **Default** `true`\\ **Text** `-`\\ **MDE** `-`\\ **API** `RELEASED_FOR_KEY_USER_APPS` abenabap.html abencds.html abencds\_annotations.html abencds\_annotations\_sap.html abencds\_annotations\_ktd\_docu.html abencds\_frmwrk\_annos.html abencds\_analytics\_f.html