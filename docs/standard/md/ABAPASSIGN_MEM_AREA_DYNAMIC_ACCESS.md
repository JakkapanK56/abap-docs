...
description: |
  ABAPASSIGN_MEM_AREA_DYNAMIC_ACCESS - Standard ABAP language reference documentation
library: "standard"
...
If a class of another program is specified in `clif_name` using an [absolute type name](ABENABSOLUTE_TYPENAME_GLOSRY.html), this program is loaded into a new [additional program group](ABENADDITIONAL_PROG_GROUP_GLOSRY.html) or into the current [program group](ABENPROGRAM_GROUP_GLOSRY.html), depending on the program type, if it has not yet been loaded. If required, the [program constructor](ABENPROGRAM_CONSTRUCTOR_GLOSRY.html) is also executed in this case.
