...
description: |
  ABENLOGEXP_SELECT_OPTION - Standard ABAP language reference documentation
library: "standard"
...
-   If the [selection table](ABENSELECTION_TABLE_GLOSRY.html)\\ `selcrit` was declared as a special form of ranges table for the data object `operand` using the statement [`SELECT-OPTIONS selcrit`](ABAPSELECT-OPTIONS.html), the comparison expression can also be specified in an [obsolete short form](ABENLOGEXP_IN_SHORT_FORM.html).

-   [Comparison with Ranges Table](ABENLOGEXP_IN_RANGES_ABEXA.html)
-   [Comparison with Selection Table](ABENLOG_EXP_IN_ABEXA.html)
