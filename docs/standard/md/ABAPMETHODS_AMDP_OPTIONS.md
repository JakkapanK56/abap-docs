...
description: |
  ABAPMETHODS_AMDP_OPTIONS - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPMETHODS_AMDP_OPTIONS.htm"
...
The option `READ-ONLY` must be specified at least once in the implementation of an [AMDP function](ABENAMDP_FUNCTION_GLOSRY.html) or an [L procedure](ABENAMDP_HDB_L_INTERNAL.html). If the addition `AMDP OPTIONS` is used in the declaration of a method with a `RETURNING` parameter, the option `READ-ONLY` must already be specified in the declaration.
