...
description: |
  The class `CL_DEMO_CDS_AUTH_PFCG_MAPPING` accesses the view `DEMO_CDS_AUTH_PFCG_MAPPING` and an SQL trace(ABENSQL_TRACE_GLOSRY.html) demonstrates how the PFCG mapping works. -   The element list of the left side is a comma-separated list of one or more directly specified CDS elements(ABENCDS_ELE
library: "standard"
...
The class `CL_DEMO_CDS_AUTH_PFCG_MAPPING` accesses the view `DEMO_CDS_AUTH_PFCG_MAPPING` and an [SQL trace](ABENSQL_TRACE_GLOSRY.html) demonstrates how the PFCG mapping works.
