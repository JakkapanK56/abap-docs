...
description: |
  ABENDDIC_BUILTIN_TYPES_INT_PACK - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "ui"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENDDIC_BUILTIN_TYPES_INT_PACK.htm"
...
-   The output formats for fields on dynpros and Web Dynpros and in the statements [`WRITE`](ABAPWRITE-.html) or [`WRITE TO`](ABAPWRITE_TO.html) can be predefined for the numeric types specific to each user in the [user master record](ABENUSER_MASTER_RECORD_GLOSRY.html). The output length required for formatted output is usually greater than the number of places in ABAP Dictionary. If the output length is too short, any formatting characters are suppressed.
-   If a [dynpro field](ABENDYNPRO_FIELD_GLOSRY.html) has one of the built-in numeric types, the output of a sign must be defined in a [domain](ABENDDIC_DOMAINS.html) so that negative values can be represented.
