...
description: |
  ABAPDERIVED_TYPES_OWN - Standard ABAP language reference documentation
library: "standard"
...
The following source code section taken from the `BP_DEMO_MANAGED_ROOT_REPORTED=CCIMP` of the global class of the [behavior pool](ABENBEHAVIOR_POOL_GLOSRY.html) `BP_DEMO_MANAGED_ROOT_REPORTED` demonstrates the component `%own` in the context of a method implementation for a [validation](ABENBDL_VALIDATIONS.html) that checks field values of RAP BO instances before saving. See the executable program for this example further down.
