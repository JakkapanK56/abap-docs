---
title: "ABENCDS_OPERANDS_AND_EXPR_V2"
description: |
  ABENCDS_OPERANDS_AND_EXPR_V2 - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCDS_OPERANDS_AND_EXPR_V2.htm"
abapFile: "ABENCDS_OPERANDS_AND_EXPR_V2.html"
keywords: ["select", "case", "data", "ABENCDS", "OPERANDS", "AND", "EXPR"]
---

-   [Elementary operands](ABENCDS_OPERANDS_V2.html):

-   [Literals](ABENCDS_LITERAL_V2.html)
-   [Fields of data sources](ABENCDS_FIELD_V2.html)
-   [Parameters](ABENCDS_PARAMETER_V2.html)
-   [Session variables](ABENCDS_SESSION_VARIABLE_V2.html)

-   [Expressions](ABENCDS_EXPRESSIONS_V2.html):

-   [Arithmetic expressions](ABENCDS_ARITHMETIC_EXPRESSION_V2.html)
-   [Aggregate expressions](ABENCDS_AGGREGATE_FUNCTIONS_V2.html)
-   [Case distinction](ABENCDS_CASE_EXPRESSION_V2.html)
-   [Casting](ABENCDS_CAST_EXPRESSION_V2.html)
-   [Built-in Functions](ABENCDS_BUILTIN_FUNCTIONS_V2.html)

-   [Conditions](ABENCDS_CONDITIONAL_EXPRESSION_V2.html)
-   [Path expressions](ABENCDS_PATH_EXPRESSION_V2.html)
-   [Passing actual parameters to input parameters](ABENCDS_SELECT_PARAMETERS_V2.html)

abenabap.html abencds.html abencds\_entities.html abencds\_f1\_ddl\_syntax.html abencds\_view\_entity.html abencds\_v2\_views.html abencds\_define\_view\_entity.html abencds\_select\_statement\_v2.html