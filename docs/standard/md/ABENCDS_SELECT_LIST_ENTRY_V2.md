...
description: |
  ABENCDS_SELECT_LIST_ENTRY_V2 - Standard ABAP language reference documentation
library: "standard"
...
-   [`include aspect`](ABENCDS_INCLUDE_ASPECT.html) includes 1 to n elements of a [CDS aspect](ABENCDS_DDL_ASPECT_GLOSRY.html).
...
-   With respect to the element annotations, publications of a CDS association [`association`](ABENCDS_SELECT_LIST_ASSOCIATION_V2.html) are handled like any regular element. When a CDS association defined in a different CDS view is exposed, the annotations of preceding publications are inherited when [annotations are evaluated](ABENCDS_ANNOTATIONS_ANALYSIS.html) using the class `CL_DD_DDL_ANNOTATION_SERVICE`.
