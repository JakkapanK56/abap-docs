...
description: |
  ABAPCLASS_FOR_TESTING - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "oop"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPCLASS_FOR_TESTING.htm"
...
The source code of a test class is not part of the production code of its program and is not generated in production systems (controlled using the [profile parameter](ABENPROFILE_PARAMETER_GLOSRY.html)\\ `abap/test_generation`) . The code of such a class is not counted in code coverage measurements.

These additions assign test properties to a test class.

During test execution, the test properties are checked against values that are maintained by administration in transaction `SAUNIT_CLIENT_SETUP`.
...
-   Tests whose risk level is higher than specified by administration in transaction `SAUNIT_CLIENT_SETUP` are not executed.
...
-   For each of these expected execution times, upper limits are specified by administration in transaction `SAUNIT_CLIENT_SETUP`. These limits are usually greater than the expected execution times in order to include processes, for example compilations, that are not part of the test itself. If a test exceeds the upper limit associated with the expected time, the system behavior depends on the execution mode of the test and whether the ABAP Unit runtime monitoring is switched on or off. For more information about execution mode and runtime monitoring, see the documentation of transaction `SAUNIT_CLIENT_SETUP` or [ABAP Unit](https://help.sap.com/docs/ABAP_PLATFORM_NEW/ba879a6e2ea04d9bb94c7ccd7cdac446/491cfd8926bc14cde10000000a42189b) in SAP Help Portal.
...
-   The additions `RISK LEVEL` and `DURATION` replace the [pseudo comments for test classes](ABENPSEUDO_COMMENT_TEST_CLASS.html). Existing pseudo comments are not ignored but should no longer be used.
