...
description: |
  ABENNEWS-760-ABAP_SQL - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "database"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENNEWS-760-ABAP_SQL.htm"
...
Until ABAP release 7.60, a [CDS entity](ABENCDS_ENTITY_GLOSRY.html) could only be used together with database tables and DDIC views in a [`SELECT`](ABAPSELECT.html) statement if addressed using its [CDS-managed DDIC view (obsolete)](ABENCDS_MNGDDDIC_VIEW_GLOSRY.html). This restriction now no longer applies. From ABAP release 7.60, CDS views can be addressed using the name of their [CDS entity](ABENCDS_ENTITY_GLOSRY.html), even if linked with database tables or DDIC views using [joins](ABENJOIN_GLOSRY.html) or [subqueries](ABENSUBQUERY_GLOSRY.html). Accessing a CDS view by specifying the CDS-managed DDIC view (obsolete) is [obsolete](ABENABAP_SQL_CDS_OBSOLETE.html).

If one the new features listed here is used in an ABAP SQL statement, the syntax check is performed in a [strict mode](ABENABAP_SQL_STRICTMODE_760.html), which handles the statement more strictly than the regular syntax check.
