...
description: |
  ABENABAP_SQL_PATH_RESTRICTIONS - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "database"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENABAP_SQL_PATH_RESTRICTIONS.htm"
...
-   Due to [implicit client handling](ABENABAP_SQL_CLIENT_HANDLING.html), no associations can be used whose `ON` condition accesses the client columns of their [association source](ABENASSOCIATION_SOURCE_GLOSRY.html) or [association target](ABENASSOCIATION_TARGET_GLOSRY.html). This cannot be bypassed using the obsolete addition [`CLIENT SPECIFIED`](ABAPSELECT_CLIENT_OBSOLETE.html) either.
...
-   The obsolete addition [`CLIENT SPECIFIED`](ABAPSELECT_CLIENT_OBSOLETE.html) cannot be used together with path expressions that contain [CDS associations](ABENCDS_ASSOCIATION_GLOSRY.html) if the [association target](ABENASSOCIATION_TARGET_GLOSRY.html) is client-dependent. In path expressions in the [`FROM`](ABAPFROM_CLAUSE.html) clause, the [association sources](ABENASSOCIATION_SOURCE_GLOSRY.html) of the associations cannot be client-dependent either.
-   The addition [`USING CLIENT`](ABAPSELECT_CLIENT.html) and the obsolete addition [`CLIENT SPECIFIED`](ABAPSELECT_CLIENT_OBSOLETE.html) cannot be combined with path expressions that contain [CDS associations](ABENCDS_ASSOCIATION_GLOSRY.html) or [CTE associations](ABENCTE_ASSOCIATION_GLOSRY.html) if they are [exposed](ABAPWITH_ASSOCIATIONS_DEFINING.html) by common table expressions.
