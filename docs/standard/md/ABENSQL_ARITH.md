...
title: "ABENSQL_ARITH"
description: |
  ABENSQL_ARITH - Standard ABAP language reference documentation
library: "standard"
...
-   If an operand is prefixed with a minus sign (`-`), the syntax check is performed in a [strict mode](ABENABAP_SQL_STRICTMODE_740_SP08.html), which handles the statement more strictly than the regular syntax check.
-   If operands of type `FLTP` or `f` are used in a decimal floating point expression or integer types are used in a binary floating point expression, the syntax check is performed in a [strict mode](ABENABAP_SQL_STRICTMODE_913.html), which handles the statement more strictly than the regular syntax check.
