...
title: "ABAPWAIT_APC"
description: |
  ABAPWAIT_APC - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPWAIT_APC.htm"
abapFile: "ABAPWAIT_APC.html"
keywords: ["update", "do", "if", "method", "data", "ABAPWAIT", "APC"]
---

[Short Reference](ABAPWAIT_SHORTREF.html)

``WAIT FOR PUSH CHANNELS           [[`MESSAGING CHANNELS`](ABAPWAIT_AMC.html)]\           [[`ASYNCHRONOUS TASKS`](ABAPWAIT_ARFC.html)]\           UNTIL [`log_exp`](ABENLOGEXP.html)\ [UP TO sec SECONDS].``
...
This variant of the statement `WAIT` is only intended for use in client programs of [ABAP Push Channels](ABENAPC.html). Any [logical expression](ABENLOGICAL_EXPRESSION_GLOSRY.html) can be specified for [`log_exp`](ABENLOGEXP.html) after `UNTIL`.
...
If these additions are specified, this variant of the statement `WAIT` is combined with the variants [`WAIT FOR MESSAGING CHANNELS`](ABAPWAIT_AMC.html) or [`WAIT FOR ASYNCHRONOUS TASKS`](ABAPWAIT_ARFC.html). The program flow described in the variants is added to the program flow described above. The `WAIT` statement then also waits for APC messages or for asynchronously called functions to be called and does not just wait for AMC messages.
...
`**sy-subrc**` **Meaning** 0 The logical expression `log_exp` is true. 4 The logical expression `log_exp` is false. Also, no receiver is registered for APC messages in the current [internal session](ABENINTERNAL_SESSION_GLOSRY.html) and no registrations for AMC messages or [asynchronous function calls](ABENASYNCHRONOUS_RFC_GLOSRY.html) exist when the additions `MESSAGING CHANNELS` or `ASYNCHRONOUS TASKS` are used. 8 The logical expression `log_exp` is false and when the addition `UP TO` was specified, the maximum time was exceeded. abenabap.html abenabap\_reference.html abenabap\_channel\_daemon.html abenabap\_channels.html abenapc.html