...
description: |
  ABENSTREAMING_CLASSES_INTERFACES - Standard ABAP language reference documentation
library: "standard"
...
In the case of data streams, the following methods are implemented as [kernel methods](ABENKERNEL_METHOD_GLOSRY.html). The actual access to the [streaming data source](ABENSTREAMING_DATA_SOURCE_GLOSRY.html) or [streaming data sink](ABENSTREAMING_DATA_SINK_GLOSRY.html) is completed by the ABAP runtime framework.
