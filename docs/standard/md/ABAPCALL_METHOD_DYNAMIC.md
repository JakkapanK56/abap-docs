...
description: |
  ABAPCALL_METHOD_DYNAMIC - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPCALL_METHOD_DYNAMIC.htm"
...
Dynamic call of the static method `GUI_DOWNLOAD` of the global class [`CL_GUI_FRONTEND_SERVICES`](ABENFRONTEND_SERVICES.html) for storing the content of an internal table in a file on the current [presentation server](ABENPRESENTATION_SERVER_GLOSRY.html). The names of the class and method are specified in the strings `class` and `meth`. The interface parameters are passed using the internal table `ptab` and return values are assigned to the exceptions of the method using the table `etab`. Exceptions that are raised at the method call itself are handled in a [`TRY`](ABAPTRY.html) control structure with statement [`CATCH`](ABAPTRY.html).
