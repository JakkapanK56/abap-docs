...
description: |
  ABENDATA_OBJECTS_STRUCTURE - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "oop"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENDATA_OBJECTS_STRUCTURE.htm"
...
-   In ABAP Dictionary, structured types are [DDIC structures](ABENDDIC_STRUCTURES.html) or types of [DDIC database tables](ABENDDIC_DATABASE_TABLES.html) or of [DDIC views](ABENDDIC_VIEWS.html).
...
-   [Do not include components from structures](ABENINCLUDING_STRUCTURES_GUIDL.html)
...
-   The target or source field of the statements [`OPEN DATASET`](ABAPOPEN_DATASET.html) and [`TRANSFER`](ABAPTRANSFER.html) cannot be a deep structure.
-   No internal tables with a deep line type can be passed to a `TABLES` parameter in a [remote function call](ABENREMOTE_FUNCTION_CALL_GLOSRY.html). Deep structures can be passed to the other parameters, as long as they do not contain any reference variables.

-   Statement [`TABLES`](ABAPTABLES.html) for table work areas.
-   Obsolete use of `LIKE` instead of `TYPE` as a reference to data types in the ABAP Dictionary in [type declarations](ABAPTYPES.html) and [typings](ABENTYPING_COMPLETE.html)
-   Obsolete use of [`INCLUDE STRUCTURE`](ABAPINCLUDE_TYPE.html)
-   Obsolete use of `STRUCTURE` to force a structure to [field symbols](ABAPFIELD-SYMBOLS.html) or interface parameters of [function modules](ABENFUNCTION_PARAMETERS.html) or [subroutines](ABAPFORM_PARAMETERS.html).
