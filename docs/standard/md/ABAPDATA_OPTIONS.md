...
description: |
  ABAPDATA_OPTIONS - Standard ABAP language reference documentation
library: "standard"
...
As well as the additions `VALUE` and `READ-ONLY`, the syntax also allows the obsolete addition [`COMMON PART`](ABAPDATA_COMMON.html).
...
[Specifying Type-Friendly Start Values](ABENSTART_VALUES_GUIDL.html)
...
-   In the [declaration part](ABENDECLARATION_PART_GLOSRY.html) of [global classes](ABENGLOBAL_CLASS_GLOSRY.html) the character representation of default values is limited to 132 characters. All positions exceeding 132 characters will be truncated when working in the form-based class builder in `SE24`. When you work in the source code-based view in `SE24` and in [ADT](ABENABAP_DEV_ENVIR_GLOSRY.html), exceeding characters will not be truncated. However, a syntax warning appears.
