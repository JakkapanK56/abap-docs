...
description: |
  ABAPMODIFY_ITAB_MULTIPLE - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "data-structures"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPMODIFY_ITAB_MULTIPLE.htm"
...
Outside of classes, an [obsolete short form](ABENITAB_SHORT_FORMS.html) is possible where `FROM wa` can be omitted if the internal table has an identically named [header line](ABENHEADER_LINE_GLOSRY.html)\\ `itab`. The statement then uses the header line as the work area implicitly. Furthermore, `USING KEY` cannot be specified without `FROM wa`.
