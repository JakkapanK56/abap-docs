...
description: |
  ABENCDS_DEFINE_ENUM_TYPE - Standard ABAP language reference documentation
library: "standard"
...
**Note** The base type is the technical storage type of [CDS enumerated elements](ABENCDS_ENUM_ELEMENT_GLOSRY.html). However, in the code element information or in the [debugger](ABENDEBUGGER_GLOSRY.html), the data type of CDS enumerated elements is displayed as *ENUM*.
...
`len` can be 1 to 8. \\ [`NUMC`](ABENDDIC_BUILTIN_TYPES.html) with length `len`. abenabap.html abencds.html abencds\_tdl.html abencds\_types.html abencds\_define\_type.html abencds\_enumeration\_types.html