...
title: "ABAPSQL_EXPR"
description: |
  ABAPSQL_EXPR - Standard ABAP language reference documentation
library: "standard"
...
-   When SQL expressions are used other than for individually specified columns or individually specified aggregate expressions, the syntax check is performed in a [strict mode as of ABAP release 7.40, SP05](ABENABAP_SQL_STRICTMODE_740_SP05.html), which handles the statement more strictly than the regular syntax check. If other SQL expressions are specified along with [aggregate expressions](ABAPSELECT_AGGREGATE.html) or the addition [`GROUP BY`](ABAPGROUPBY_CLAUSE.html), the syntax check is performed in [strict mode as of ABAP release 7.40, SP08](ABENABAP_SQL_STRICTMODE_740_SP08.html). Using [aggregate expressions](ABAPSELECT_AGGREGATE.html) as operands of SQL expressions results in the [strict mode as of ABAP release 7.74](ABENABAP_SQL_STRICTMODE_774.html).
