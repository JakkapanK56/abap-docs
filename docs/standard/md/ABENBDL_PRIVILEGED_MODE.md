...
keywords: ["update", "delete", "do", "if", "case", "method", "class", "data", "types", "ABENBDL", "PRIVILEGED", "MODE"]
---
...
The following example shows a managed BDEF based on the CDS root view entity `DEMO_RAP_PRIVILEGED`. It enables BDEF privileged mode using the deprecated syntax `with privileged mode;`. The privileged mode must be implemented in the ABAP behavior pool `BP_DEMO_RAP_PRIVILEGED`. It is currently not implemented in this example.
