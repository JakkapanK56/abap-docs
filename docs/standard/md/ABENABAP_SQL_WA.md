...
description: |
  ABENABAP_SQL_WA - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "database"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENABAP_SQL_WA.htm"
abapFile: "ABENABAP_SQL_WA.html"
keywords: ["select", "insert", "update", "delete", "do", "if", "data", "types", "internal-table", "ABENABAP", "SQL"]
---
...
The following rules apply to work areas that do not contain reference variables for [LOB handles](ABENLOB_HANDLE_GLOSRY.html) :
...
If a work area is an [LOB handle structure](ABENLOB_HANDLE_STRUCTURE_GLOSRY.html), that is, it contains at least one reference variable for [LOB handles](ABENLOB_HANDLE_GLOSRY.html), it must be structured in exactly the same way as the structure of the data source and all components except for the [LOB handle components](ABENLOB_HANDLE_COMPONENT_GLOSRY.html) must be [compatible](ABENCOMPATIBLE_GLOSRY.html) with the corresponding components of the data source. The static type of each LOB handle component must contain the `IF_ABAP_DB_LOB_HANDLE` interface and match both the current statement and the data type of the assigned LOB.
