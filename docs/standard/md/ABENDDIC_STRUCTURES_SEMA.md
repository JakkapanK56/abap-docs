...
description: |
  ABENDDIC_STRUCTURES_SEMA - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "types"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENDDIC_STRUCTURES_SEMA.htm"
...
-   A component with the directly specified type of a decimal floating point number can be assigned an [output style](ABENDDIC_DECIMAL_FLOATING_POINT.html) for dynpros.
...
-   **Key field**
-   Like table fields of DDIC database tables, structure components can be defined as [key fields](ABENDDIC_DATABASE_TABLES_KEY.html). Components of structures used as lock parameters in a [lock object](ABENLOCK_OBJECT_GLOSRY.html) must be defined as key fields. When a structure with key fields is [included in DDIC database tables](ABENDDIC_DATABASE_TABLES_TECHSTRUC.html), however, they are not used as key fields of the DDIC database table. Either all or no fields of an included structure can become key fields when included.
...
-   **Text language**
-   Like table fields of DDIC database tables, one structure component of type [`LANG`](ABENDDIC_BUILTIN_TYPES.html#@@ITOC@@ABENDDIC_BUILTIN_TYPES_7) can be labeled as the [text language](ABENDDIC_DBTAB_TEXT_LANGUAGE.html) of the structure and the same rules are applied when transporting data of that structured type between non-Unicode and Unicode Systems.
