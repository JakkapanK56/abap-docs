---
title: "ABENCLIENT_HANDLING_GUIDL"
description: |
  ABENCLIENT_HANDLING_GUIDL - Standard ABAP language reference documentation
library: "standard"
...
keywords: ["select", "do", "if", "method", "data", "ABENCLIENT", "HANDLING", "GUIDL"]
---
...
-   [ABAP SQL](ABENABAP_SQL_GLOSRY.html) statements work with implicit client handling, where the data of the current client is accessed by default. This is specified by passing an implicit condition for the current client to `WHERE` conditions, and ignoring clients specified in modifying statements in work areas. Implicit client handling in ABAP SQL can be switched to one more different clients by using the addition [`USING`](ABAPSELECT_CLIENT.html) in queries or the additions [`USING`](ABAPIUMD_CLIENT.html) or [`CLIENT SPECIFIED`](ABAPIUMD_CLIENT.html) in write statements.
-   [Native SQL](ABENNATIVE_SQL_GLOSRY.html) and [ADBC](ABENAMDP_GLOSRY.html) do not apply implicit client handling. The client in question must be selected explicitly when Native SQL or AMDP is used to access client-dependent database tables or views. In AMDP, [client-safe](ABENCLIENT_SAFE_GLOSRY.html) AMDP methods can be used, to access the current client only.
...
-   The addition `CLIENT SPECIFIED` is fully [obsolete](ABAPSELECT_CLIENT_OBSOLETE.html) in queries and partially [obsolete](ABAPUD_CLIENT_OBSOLETE.html) in write statements.

SELECT SINGLE ... \\n FROM scarr USING '...' \\n WHERE ... ... \\n INTO ... SELECT SINGLE ... \\n FROM scarr \\n WHERE ... \\n INTO ... abenabap.html abenabap\_reference.html abenabap\_pgl.html abenarchitecture\_gdl.html abendata\_storage\_gdl.html