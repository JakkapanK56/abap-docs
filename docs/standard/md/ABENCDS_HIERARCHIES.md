...
description: |
  ABENCDS_HIERARCHIES - Standard ABAP language reference documentation
library: "standard"
...
A CDS hierarchy is defined in the [DDL source code](ABENDDL_SOURCE_CODE_GLOSRY.html) of a [CDS data definition](ABENCDS_DATA_DEFINITION_GLOSRY.html) in the [ABAP development tools for Eclipse](ABENADT_GLOSRY.html) using the statement [`DEFINE HIERARCHY`](ABENCDS_F1_DEFINE_HIERARCHY.html) in the [CDS DDL](ABENCDS_DDL_GLOSRY.html) of the [ABAP Core Data Services (CDS)](ABENCDS.html). The name of the CDS hierarchy must match the name of the [DDL source code](ABENDDL_SOURCE_CODE_GLOSRY.html). DDL source code in a CDS hierarchy can also be displayed in [Repository Browser](ABENREPOSITORY_BROWSER_GLOSRY.html) in [ABAP Workbench](ABENABAP_WORKBENCH_GLOSRY.html).
