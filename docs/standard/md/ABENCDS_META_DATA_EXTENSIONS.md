...
description: |
  ABENCDS_META_DATA_EXTENSIONS - Standard ABAP language reference documentation
library: "standard"
...
keywords: ["do", "if", "try", "method", "class", "data", "ABENCDS", "META", "DATA", "EXTENSIONS"]
---
...
A CDS metadata extension is a [CDS object](ABENCDS_OBJECT_GLOSRY.html) that is defined and transported in a separate piece of [DDLX source code](ABENDDLX_SOURCE_CODE_GLOSRY.html). These pieces of DDLX source code can only be edited in the [ABAP development tools for Eclipse (ADT)](ABENADT_GLOSRY.html). DDLX source code can also be displayed in [Repository Browser](ABENREPOSITORY_BROWSER_GLOSRY.html) in [ABAP Workbench](ABENABAP_WORKBENCH_GLOSRY.html). Once a CDS metadata extension has been activated for a CDS entity, the methods of the class `CL_DD_DDL_ANNOTATION_SERVICE` take the annotations of the associated metadata extensions into account by default when [evaluating the annotations](ABENCDS_ANNOTATIONS_ANALYSIS.html) of this entity.
...
A CDS entity can have more than one CDS metadata extension. The following rules determine which metadata extension to use to [evaluate the annotations](ABENCDS_ANNOTATIONS_ANALYSIS.html):

In an evaluation with the class `CL_DD_DDL_ANNOTATION_SERVICE`, the priority is determined by the layers of the metadata extensions. Next, the direct and indirect annotations of the CDS entity itself are added that do not have an entry in a metadata extension.
...
-   Metadata extensions support the [separation of concerns (SoC)](ABENSEPARATION_CONCERNS_GUIDL.html) concept.
