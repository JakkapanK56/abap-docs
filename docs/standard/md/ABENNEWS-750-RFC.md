---
title: "ABENNEWS-750-RFC"
description: |
  ABENNEWS-750-RFC - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENNEWS-750-RFC.htm"
abapFile: "ABENNEWS-750-RFC.html"
keywords: ["do", "if", "method", "class", "ABENNEWS", "750", "RFC"]
---
...
The methods of the class `CL_DYNAMIC_DESTINATION` are used to manage dynamic [RFC destinations](ABENRFC_DESTINATION.html) in ABAP release 7.50 and higher. In particular, the method `CREATE_RFC_DESTINATION` makes it possible to create a dynamic destination, which can be used in the current sessions for RFCs.

Alongside the general publish-and-subscribe mechanism, a point-to-point communication option was introduced for the [ABAP messaging channels (AMC)](ABENAMC.html). Here, a sender object addresses precisely one receiver session. The send action can be either synchronous or asynchronous. This is done using the new factory method `CREATE_MESSAGE_PRODUCER_BY_ID` of the class `CL_AMC_CHANNEL_MANAGER`. The ID of a receiver session is provided by the method `GET_CONSUMER_SESSION_ID` of the same class.

The framework for [ABAP channels](ABENAPC.html) was expanded as follows:
...
-   A [non-blocking](ABENAPC_NON_BLOCKING_MODEL.html) model was introduced for stateful APC applications.
