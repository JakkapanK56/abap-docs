...
description: |
  ABAPMESSAGE_INTO - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPMESSAGE_INTO.htm"
abapFile: "ABAPMESSAGE_INTO.html"
keywords: ["do", "if", "data", "types", "ABAPMESSAGE", "INTO"]
---
...
The statement [`MESSAGE`](ABAPMESSAGE.html) with the addition `INTO` assigns the short text of the message to the target field `text`. The [message type](ABENMESSAGE_TYPE_GLOSRY.html) is irrelevant. The program flow is not interrupted and no [message processing](ABENABAP_MESSAGES_TYPES.html) takes place. The following can be specified for `text`:

The addition `INTO` cannot be specified when a text [`text`](ABAPMESSAGE_TEXT.html) is specified or when an object reference variable is specified in [`msg`](ABAPMESSAGE_TEXT.html).
