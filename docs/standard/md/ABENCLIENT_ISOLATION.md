...
description: |
  ABENCLIENT_ISOLATION - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCLIENT_ISOLATION.htm"
abapFile: "ABENCLIENT_ISOLATION.html"
keywords: ["select", "do", "if", "method", "class", "data", "ABENCLIENT", "ISOLATION"]
---
...
Other ways for accessing client-dependent data, as [Native SQL](ABENNATIVE_SQL_GLOSRY.html), are not supported in ABAP for Cloud Development.

-   Directly with [ABAP SQL](ABENABAP_SQL_GLOSRY.html).
-   In ABAP SQL, implicit [client handling](ABENCLIENT_HANDLING_GLOSRY.html) takes place that accesses data of the current client only. In ABAP for Cloud Development, it is not possible to change that behavior as it can be done in Standard ABAP with the addition [`USING CLIENT`](ABAPSELECT_CLIENT.html) or the obsolete addition [`CLIENT SPECIFIED`](ABAPSELECT_CLIENT_OBSOLETE.html).
