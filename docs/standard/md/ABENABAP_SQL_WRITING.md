...
description: |
  ABENABAP_SQL_WRITING - Standard ABAP language reference documentation
library: "standard"
...
-   The latter is also true using an [`OPEN CURSOR`](ABAPOPEN_CURSOR.html).
-   Write access to DDIC database tables or DDIC table views in the ABAP Dictionary with [replacement objects](ABENDDIC_REPLACEMENT_OBJECTS.html) is still performed on the DDIC database table or the DDIC view and produce a syntax warning.
