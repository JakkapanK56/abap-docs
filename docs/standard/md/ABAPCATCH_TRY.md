...
title: "ABAPCATCH_TRY"
description: |
  ABAPCATCH_TRY - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPCATCH_TRY.htm"
abapFile: "ABAPCATCH_TRY.html"
keywords: ["select", "delete", "while", "if", "case", "try", "catch", "method", "class", "data", "types", "exception-handling", "ABAPCATCH", "TRY"]
---
...
[1. `... BEFORE UNWIND`](#ABAP_ADDITION_1@3@)

[2. `... INTO oref`](#ABAP_ADDITION_2@3@)
...
-   The statements [`CALL TRANSACTION`](ABAPCALL_TRANSACTION.html), [`CALL DIALOG`](ABAPCALL_DIALOG.html), [`SUBMIT`](ABAPCALL_TRANSACTION.html), [`STOP`](ABAPSTOP.html), [`REJECT`](ABAPREJECT.html), [`CHECK SELECT-OPTIONS`](ABAPCHECK_SELECT-OPTIONS.html), and the obsolete variant of [`LEAVE`](ABAPLEAVE-.html) are forbidden by the syntax, as they might leave the [internal session](ABENINTERNAL_SESSION_GLOSRY.html).
-   Statements such as [`LEAVE TO TRANSACTION`](ABAPLEAVE_TO_TRANSACTION.html), which are statically known to leave the internal session, are allowed. In such cases, exception handling is properly terminated while executing the `CLEANUP` blocks before the statement is executed.
-   Procedure calls are allowed. However, if the context is deleted during such a procedure call, a runtime error `EXCP_HANDLER_FAILED_TO_UNWIND` occurs. This is also the case if a statement is executed here that is allowed directly in the `CATCH` block. For example, [`LEAVE TO TRANSACTION`](ABAPLEAVE_TO_TRANSACTION.html) can be executed directly in the `CATCH` block, but not in a procedure that is called there, because otherwise the context would be deleted without executing the `CLEANUP` blocks.
