...
description: |
  ABAPUPDATE_SOURCE - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPUPDATE_SOURCE.htm"
...
[Views](ABENVIEW_GLOSRY.html) whose fields are all key fields should not be written to using `UPDATE FROM`. For [DDIC database views](ABENDATABASE_VIEW_GLOSRY.html), this produces a syntax check warning and for [DDIC projection views](ABENDDIC_PROJ_VIEW_GLOSRY.html) a syntax error or runtime error.
...
-   Instead of using commas, blanks can be used to separate set expressions in an [obsolete form](ABENABAP_SQL_LISTS_OBSOLETE.html). Commas must be specified, however, in the [strict modes](ABENABAP_SQL_STRICT_MODES.html) of the syntax check from ABAP release 7.40, SP05.
...
-   If the DDIC database table or DDIC view is specified statically, a [short form](ABAPUPDATE_OBSOLETE.html) can be specified outside of classes. This means that the work area specified using `FROM wa` can be omitted. The prerequisite is that a [table work area](ABENTABLE_WORK_AREA_GLOSRY.html)\\ `dbtab` for the corresponding DDIC database table or the DDIC view is declared using the statement `TABLES`. The system then adds the addition `FROM dbtab` implicitly to the statement `UPDATE`.
