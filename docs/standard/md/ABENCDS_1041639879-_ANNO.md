---
title: "ABENCDS_1041639879-_ANNO"
description: |
  ABENCDS_1041639879-_ANNO - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCDS_1041639879-_ANNO.htm"
abapFile: "ABENCDS_1041639879-_ANNO.html"
keywords: ["delete", "do", "if", "case", "try", "data", "ABENCDS", "1041639879", "ANNO"]
---
...
The "mandatory" flag is a hint to the “Filter UI” to enforce a user entry for a (semantically) mandatory element even if a default value exists (e.g. being defined via a CASE expression).

If the user deletes the proposal for a mandatory filter in the Filter UI the UI will show an error message "please enter a value for filter ...". Finally, the entered value is send to the engine. So the Filter UI makes sure that the engine's automatic replacement is never done for mandatory filters.

**Kind** [Framework-specific annotation](ABENFRMWRK_ANNOTATION_GLOSRY.html)\\ **Syntax** `Consumption.filter.mandatory`\\ **Scope** `#ELEMENT`\\ **Type** `Boolean`\\ **Enums** `-`\\ **Default** `true`\\ **Text** `-`\\ **MDE** `X`\\ **API** `RELEASED_FOR_KEY_USER_APPS, RELEASED_FOR_SAP_CLOUD_PLATFORM` abenabap.html abencds.html abencds\_annotations.html abencds\_annotations\_sap.html abencds\_annotations\_ktd\_docu.html abencds\_frmwrk\_annos.html abencds\_consumption\_f.html