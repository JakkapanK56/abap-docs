...
description: |
  ABENESCAPE_FUNCTIONS - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENESCAPE_FUNCTIONS.htm"
...
`format` expects data objects of the type `i`. An invalid value for `format` raises an exception of the class `CX_SY_STRG_PAR_VAL`. For all characters whose codes are between *x00* and *xFF*, the program `DEMO_ESCAPE` demonstrates the effect of all associated formats from the class `CL_ABAP_FORMAT`. The top line contains the names of the constants from the class `CL_ABAP_FORMAT` without the prefix *E\_*. The remaining lines show the effect on the characters specified in the first two columns.
...
The program `DEMO_ESCAPE_MARKUP` demonstrates the replacement rules for markup languages.
...
The program `DEMO_ESCAPE_URL_URI` demonstrates the replacement rules for URLs and URIs.
...
The program `DEMO_ESCAPE_JSON` demonstrates the replacement rules of the format `E_JSON_STRING` for [JSON](ABENJSON_GLOSRY.html).
...
The program `DEMO_ESCAPE_REGEX` demonstrates the replacement rules for the formats `E_PCRE` (PCRE regular expressions), `E_PCRE_EXTENDED` (PCRE regular expressions, compiled in extended mode), and `E_REGEX` (POSIX regular expressions, obsolete).
...
The program `DEMO_ESCAPE_STRING_TEMPLATE` demonstrates the replacement rules of the format `E_STRING_TPL` for string templates.
...
The program `DEMO_ESCAPE_XSS` demonstrates the replacement rules of the formats `E_XSS_...`

The result of the following escaping is *German&#x20;umlauts&#x3a;&#x20;&#x22;&#xe4;&#xf6;&#xfc;&#x22;*.

[String Functions, `escape` for XSS](ABENSTRING_FUNCTION_ESC_XSS_ABEXA.html)
