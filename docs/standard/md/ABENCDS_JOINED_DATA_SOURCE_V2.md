...
-   There is no limit on the number of join expressions in a `SELECT` statement of a CDS view entity in the DDL, but there is an [ATC](ABENATC_GLOSRY.html) check that produces a message once a specific number of expressions is reached.
