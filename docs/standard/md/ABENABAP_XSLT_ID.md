...
description: |
  ABENABAP_XSLT_ID - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENABAP_XSLT_ID.htm"
...
-   The executable example [Transformation of XML Element Names](ABENCALL_TRAFO_UPPER_LOWER_ABEXA.html) demonstrates how to modify [XML](ABENABAP_JSON_NAMES_TO_UPPER_ABEXA.html) data and the example [JSON, Transform Names](ABENABAP_JSON_NAMES_TO_UPPER_ABEXA.html) demonstrates this for JSON data.
-   If the predefined identity transformation `ID` is copied to a different XSLT program, the target program cannot be used to the same extent, since in this case no internal special handling takes place. A non-optimized copy of `ID` is executed in accordance with its source code as an [XSL](ABENDOM_GLOSRY.html) transformation on a [Document Object Model (DOM)](ABENDOM_GLOSRY.html) from the iXML library. More specifically, JSON data is not handled in the same way as with `ID` and usually raises an exception.
