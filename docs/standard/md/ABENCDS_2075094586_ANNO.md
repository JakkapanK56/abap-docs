...
description: |
  The release contract `C2` guarantees a stable interface for use as remote API. Defines the usage of an annotation in CDS entities with release contract `C2`: -   either right-click on its Data Definition in the Project Explorer and choose option API State, -   or open the Properties view o
library: "standard"
libraryName: "Standard ABAP"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCDS_2075094586_ANNO.htm"
abapFile: "ABENCDS_2075094586_ANNO.html"
keywords: ["do", "if", "data", "ABENCDS", "2075094586", "ANNO"]
---
...
Defines the usage of an annotation in CDS entities with release contract `C2`:
...
**Kind** [ABAP annotation](ABENABAP_ANNOTATION_GLOSRY.html)\\ **Syntax** `CompatibilityContract.c2.usageAllowed`\\ **Scope** `#ANNOTATION`\\ **Type** `Boolean`\\ **Enums** `-`\\ **Default** `true`\\ **Text** `-`\\ **MDE** `-`\\ **API** `-` abenabap.html abencds.html abencds\_annotations.html abencds\_annotations\_sap.html abencds\_annotations\_ktd\_docu.html abencds\_abap\_annos.html abencds\_compatibilitycontract\_a.html