...
description: |
  ABENSAP_PUFFERING - Standard ABAP language reference documentation
library: "standard"
...
keywords: ["select", "do", "if", "case", "method", "class", "data", "types", "ABENSAP", "PUFFERING"]
---
...
[Native SQL](ABENNATIVE_SQL_GLOSRY.html) access or [AMDP methods](ABENAMDP_METHOD_GLOSRY.html) are applied to the database directly and therefore always bypass table buffering.
...
-   For more information, see [DDIC - Database Views](ABENDDIC_DATABASE_VIEWS.html).
...
-   The latter is also true if the tables views can be accessed using [Native SQL](ABENNATIVE_SQL_GLOSRY.html) and if native SQL statements modify data in buffered tables or views.
-   Except in the case of ABAP SQL database access, table buffering also applies to database access from classic [dynpros](ABENDYNPRO_GLOSRY.html).
-   A user-defined buffering of formatted data in the shared memory is possible using [shared objects](ABENSHARED_OBJECTS_GLOSRY.html).
