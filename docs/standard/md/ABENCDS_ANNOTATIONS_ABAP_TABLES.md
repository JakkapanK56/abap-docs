...
description: |
  ABENCDS_ANNOTATIONS_ABAP_TABLES - Standard ABAP language reference documentation
library: "standard"
...
keywords: ["update", "do", "case", "try", "class", "data", "types", "ABENCDS", "ANNOTATIONS", "ABAP", "TABLES"]
---
...
See also [ABAP Annotation Definitions](ABENCDS_ANNOTATIONS_ABAP_DDLA.html).

**Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`AbapAnnotation.definitionHidden`](ABENCDS_1616694372-_ANNO.html) `#ANNOTATION` `Boolean` `-` `true` `-` `-` `-` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`AbapCatalog.buffering.numberOfKeyFields`](ABENCDS_1619044161-_ANNO.html) `#VIEW` `Integer` `-` `000` `-` `-` `RELEASED_FOR_SAP_CLOUD_PLATFORM`\\ [`AbapCatalog.buffering.status`](ABENCDS_1242119123-_ANNO.html) `#VIEW` `String(20)` `#ACTIVE` \\
`#NOT_ALLOWED` \\
...
`RELEASED_FOR_SAP_CLOUD_PLATFORM`\\ [`API.state[ ]`](ABENCDS_213745370-_ANNO.html) `#ANNOTATION` `String(40)` `#NOT_RELEASED` \\
...
`RELEASED_FOR_SAP_CLOUD_PLATFORM` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`CompatibilityContract.c0.allowedChanges.annotation[ ]`](ABENCDS_1748011752_ANNO.html) `#ANNOTATION` `String(20)` `#ADD` \\
`#ANY` \\
`#CUSTOM` \\
`#NONE` \\
`#REMOVE` `-` `-` `-` `-`\\ [`CompatibilityContract.c0.allowedChanges.value[ ]`](ABENCDS_1655423479-_ANNO.html) `#ANNOTATION` `String(20)` `#ADD` \\
`#ANY` \\
`#CUSTOM` \\
`#FALSE_TO_TRUE` \\
`#NONE` \\
`#REMOVE` \\
`#TRUE_TO_FALSE` \\
`#UPDATE` `-` `-` `-` `-`\\ [`CompatibilityContract.c0.usageAllowed`](ABENCDS_1698583913-_ANNO.html) `#ANNOTATION` `Boolean` `-` `true` `-` `-` `-`\\ [`CompatibilityContract.c1.allowedChanges.annotation[ ]`](ABENCDS_113021353_ANNO.html) `#ANNOTATION` `String(20)` `#ADD` \\
`#ANY` \\
`#CUSTOM` \\
`#NONE` \\
`#REMOVE` `-` `-` `-` `-`\\ [`CompatibilityContract.c1.allowedChanges.value[ ]`](ABENCDS_1971561783-_ANNO.html) `#ANNOTATION` `String(20)` `#ADD` \\
`#ANY` \\
`#CUSTOM` \\
`#FALSE_TO_TRUE` \\
`#NONE` \\
`#REMOVE` \\
`#TRUE_TO_FALSE` \\
`#UPDATE` `-` `-` `-` `-`\\ [`CompatibilityContract.c1.usageAllowed`](ABENCDS_130960353_ANNO.html) `#ANNOTATION` `Boolean` `-` `true` `-` `-` `-`\\ [`CompatibilityContract.c2.allowedChanges.annotation[ ]`](ABENCDS_1255621526-_ANNO.html) `#ANNOTATION` `String(20)` `#ADD` \\
`#ANY` \\
`#CUSTOM` \\
`#NONE` \\
`#REMOVE` `-` `-` `-` `-`\\ [`CompatibilityContract.c2.allowedChanges.value[ ]`](ABENCDS_1291520119-_ANNO.html) `#ANNOTATION` `String(20)` `#ADD` \\
`#ANY` \\
`#CUSTOM` \\
`#FALSE_TO_TRUE` \\
`#NONE` \\
`#REMOVE` \\
`#TRUE_TO_FALSE` \\
`#UPDATE` `-` `-` `-` `-`\\ [`CompatibilityContract.c2.usageAllowed`](ABENCDS_2075094586_ANNO.html) `#ANNOTATION` `Boolean` `-` `true` `-` `-` `-` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`DataAging.noAgingRestriction`](ABENCDS_128703388_ANNO.html) `#TABLE_FUNCTION` \\
\\
`#VIEW` `Boolean` `-` `true` `-` `-` `RELEASED_FOR_KEY_USER_APPS` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`EndUserText.heading`](ABENCDS_92083118_ANNO.html) `#SIMPLE_TYPE` `String(55)` `-` `-` `X` `-` `RELEASED_FOR_KEY_USER_APPS` \\
...
\\
`#PARAMETER` `Boolean` `-` `true` `-` `-` `RELEASED_FOR_SAP_CLOUD_PLATFORM`\\ [`Environment.sql.passValueForClient`](ABENCDS_837403882-_ANNO.html) `#ENTITY` `Boolean` `-` `true` `-` `-` `NOT_RELEASED`\\ [`Environment.systemField`](ABENCDS_1937921420_ANNO.html) `#PARAMETER` `String(20)` `#CLIENT` \\
`#SYSTEM_DATE` \\
`#SYSTEM_LANGUAGE` \\
`#SYSTEM_TIME` \\
`#USER` \\
...
`RELEASED_FOR_SAP_CLOUD_PLATFORM` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`LanguageDependency.maxLength`](ABENCDS_1997686578_ANNO.html) `#ANNOTATION` `Integer` `-` `-` `-` `-` `-` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`MappingRole`](ABENCDS_984551930-_ANNO.html) `#ROLE` `Boolean` `-` `true` `-` `-` `RELEASED_FOR_KEY_USER_APPS` \\
...
`RELEASED_FOR_SAP_CLOUD_PLATFORM` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`MetadataExtension.usageAllowed`](ABENCDS_1537649007_ANNO.html) `#ANNOTATION` `Boolean` `-` `true` `-` `-` `-` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`ObjectModel.query.implementedBy`](ABENCDS_763589850-_ANNO.html) `#CUSTOM_ENTITY` \\
\\
`#ENTITY` `String(255)` `-` `-` `-` `-` `RELEASED_FOR_SAP_CLOUD_PLATFORM`\\ [`ObjectModel.upperCase`](ABENCDS_1673459597_ANNO.html) `#ELEMENT` \\
\\
`#SIMPLE_TYPE` `Boolean` `-` `true` `-` `-` `-`\\ [`ObjectModel.usageType.dataClass`](ABENCDS_1091668137-_ANNO.html) `#ENTITY` \\
\\
...
`RELEASED_FOR_SAP_CLOUD_PLATFORM`\\ [`ObjectModel.virtualElementCalculatedBy`](ABENCDS_1555784333_ANNO.html) `#ELEMENT` `String(255)` `-` `-` `-` `-` `RELEASED_FOR_SAP_CLOUD_PLATFORM` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`Scope[ ]`](ABENCDS_1049724201-_ANNO.html) `#ANNOTATION` `String(20)` `#ACCESSPOLICY` \\
`#ANNOTATE` \\
`#ANNOTATION` \\
`#ASPECT` \\
`#CUSTOM_ENTITY` \\
...
`#TABLE_ENTITY` \\
`#TABLE_FUNCTION` \\
`#VIEW` `-` `-` `-` `-` **Syntax** **Scope** **Type** **Enums** **Default** **Text** **MDE** **API**\\ [`Semantics.amount.currencyCode`](ABENCDS_689127610-_ANNO.html) `#ELEMENT` `ElementRef` `-` `-` `-` `-` `RELEASED_FOR_KEY_USER_APPS` \\
