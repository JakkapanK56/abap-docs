...
description: |
  ABENCDS_LOGICAL_EXTERNAL_SCHEMA - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCDS_LOGICAL_EXTERNAL_SCHEMA.htm"
...
A logical external schema can be bound to one or more external entities. For dynamic external entities, the binding is done using the addition [`PROVIDED BY`](ABAPSELECT_DATA_SOURCE.html) in ABAP SQL when accessing the external entity.
...
-   In on-premise systems, the [transaction](ABENTRANSACTION_CODE_GLOSRY.html)\\ `SDEXTERNALENTITY` can be used to configure the logical external schema.
