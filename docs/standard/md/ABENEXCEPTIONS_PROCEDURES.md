...
description: |
  ABENEXCEPTIONS_PROCEDURES - Standard ABAP language reference documentation
library: "standard"
libraryName: "Standard ABAP"
category: "error-handling"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENEXCEPTIONS_PROCEDURES.htm"
abapFile: "ABENEXCEPTIONS_PROCEDURES.html"
keywords: ["select", "do", "if", "case", "try", "method", "class", "data", "ABENEXCEPTIONS", "PROCEDURES"]
---
...
In methods and subroutines, the addition `RAISING` of the statements [`METHODS`](ABAPMETHODS.html) and [`FORM`](ABAPFORM.html) is used for the declaration. In function modules and methods of global classes, class-based exceptions are declared in the interface by selecting the Exception Class checkbox on the relevant tab page in the [Class Builder](ABENCLASS_BUILDER_GLOSRY.html) or [Function Builder](ABENFUNCTION_BUILDER_GLOSRY.html). The addition `RAISING` then appears as a [comment](ABENFUNCTION.html) of the statement [`FUNCTION`](ABAPFUNCTION.html). The declaration of an exception class in an interface is polymorphous. It declares all subclasses simultaneously.
...
The highest level of a program into which the exceptions can be propagated are processing blocks without local [data areas](ABENDATA_AREA_GLOSRY.html), namely event blocks or dialog modules. Here, all exceptions that are raised must be handled, since otherwise a runtime error occurs.

When exceptions are propagated from procedures, the following restrictions apply:

Programs called using [`SUBMIT ... AND RETURN`](ABAPSUBMIT.html) or [`CALL TRANSACTION`](ABAPCALL_TRANSACTION.html) cannot propagate any exceptions to the caller, since exception objects are bound to the [internal session](ABENINTERNAL_SESSION_GLOSRY.html) of a program.
