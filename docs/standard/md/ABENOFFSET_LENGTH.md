...
description: |
  ABENOFFSET_LENGTH - Standard ABAP language reference documentation
library: "standard"
...
-   Specification of `dobj[+off][(len)]` as an actual parameter for input parameters when calling [methods](ABENMETHOD_CALLS.html), [function modules](ABAPCALL_FUNCTION.html), and [subroutines](ABAPPERFORM.html).
...
-   The compiler always interprets such an entry as a [number](ABAPCNT.html) or [sum](ABAPSUM.html) of a field `len` in the group level processing of an [extract dataset](ABENEXTRACT_DATASET_GLOSRY.html).
-   [Obsolete syntax forms](ABENTOKENIZATION_OBSOLETE.html) can still occur outside of classes in connection with substring accesses.
-   The statement [`MOVE PERCENTAGE`](ABAPMOVE_PERCENTAGE.html) represents an obsolete form of substring access.
