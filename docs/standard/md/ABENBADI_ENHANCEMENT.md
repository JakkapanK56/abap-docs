...
description: |
  ABENBADI_ENHANCEMENT - Standard ABAP language reference documentation
library: "standard"
...
keywords: ["select", "do", "if", "method", "class", "data", "ABENBADI", "ENHANCEMENT"]
---

BAdIs are part of the [enhancements](ABENENHANCEMENT_GLOSRY.html) made available by the [ABAP Enhancement Framework](ABENENHANCEMENT_FRAMEWORK_GLOSRY.html). They are created in the [Enhancement Builder](ABENENHANCEMENT_BUILDER_GLOSRY.html). BAdIs, together with their calling positions in ABAP programs, form explicit [enhancement options](ABENENHANCEMENT_POINT_GLOSRY.html) of these programs and are assigned to [enhancement spots](ABENENHANCEMENT_SPOT_GLOSRY.html).
...
Unlike the previous classic BAdIs that were processed directly in the [BAdI Builder](ABENBADI_BUILDER_GLOSRY.html), the BAdIs of the ABAP Enhancement Framework are supported by the ABAP runtime framework by the following ABAP statements:
...
-   For [AMDP procedure implementations](ABENAMDP_PROCEDURE_METHOD_GLOSRY.html), special [AMDP BAdIs](ABENAMDP_BADI_GLOSRY.html) are available. These apply the effect of the switches from the [Switch Framework](ABENSWITCH_FRAMEWORK_GLOSRY.html) to the implementation of database procedures in the current database.
