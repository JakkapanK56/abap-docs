...
description: |
  ABENABAP_SQL_PATH_FILTER - Standard ABAP language reference documentation
library: "standard"
...
If the cardinality is specified in a path expression, the syntax check is performed in [strict mode from ABAP release 7.91](ABENABAP_SQL_STRICTMODE_791.html).
...
If the type of the join is specified in a path expression, the syntax check is performed in [strict mode from ABAP release 7.69](ABENABAP_SQL_STRICTMODE_769.html).
...
-   If a filter condition is specified in a path expression, the syntax check is performed in [strict mode from ABAP release 7.67](ABENABAP_SQL_STRICTMODE_767.html).
