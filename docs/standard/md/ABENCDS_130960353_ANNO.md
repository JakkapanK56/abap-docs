...
description: |
  The release contract `C1` guarantees a stable interface for system-internal use within AS ABAP. Defines the usage of an annotation in CDS entities with release contract `C1`: -   either right-click on its Data Definition in the Project Explorer and choose option API State, -   or open the P
library: "standard"
libraryName: "Standard ABAP"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABENCDS_130960353_ANNO.htm"
abapFile: "ABENCDS_130960353_ANNO.html"
keywords: ["do", "if", "data", "ABENCDS", "130960353", "ANNO"]
---
...
Defines the usage of an annotation in CDS entities with release contract `C1`:
...
**Kind** [ABAP annotation](ABENABAP_ANNOTATION_GLOSRY.html)\\ **Syntax** `CompatibilityContract.c1.usageAllowed`\\ **Scope** `#ANNOTATION`\\ **Type** `Boolean`\\ **Enums** `-`\\ **Default** `true`\\ **Text** `-`\\ **MDE** `-`\\ **API** `-` abenabap.html abencds.html abencds\_annotations.html abencds\_annotations\_sap.html abencds\_annotations\_ktd\_docu.html abencds\_abap\_annos.html abencds\_compatibilitycontract\_a.html