...
description: |
  ABENITAB_LOOP_CHANGE - Standard ABAP language reference documentation
library: "standard"
...
As a rule the replacement or clearing of the entire table body of the currently processed internal table is not allowed in the statement block of a [`LOOP`](ABAPLOOP_AT_ITAB_VARIANTS.html) statement. Such a write access to the table body can be achieved with statements such as [`CLEAR`](ABAPCLEAR.html), [`FREE`](ABAPFREE_DATAOBJECT.html), [`LOCAL`](ABAPLOCAL.html), [`REFRESH`](ABAPREFRESH_ITAB.html), [`SORT`](ABAPSORT_ITAB.html), [`DELETE ... WHERE`](ABAPDELETE_ITAB_LINES.html), and with all types of assignments.
