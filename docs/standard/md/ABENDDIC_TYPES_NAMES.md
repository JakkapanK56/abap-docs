...
description: |
  ABENDDIC_TYPES_NAMES - Standard ABAP language reference documentation
library: "standard"
...
keywords: ["do", "case", "data", "types", "ABENDDIC", "TYPES", "NAMES"]
---

The name of a DDIC data type must stick to the following rules:

All DDIC data types, alongside [DDIC database tables](ABENDDIC_DATABASE_TABLES.html) and [DDIC views](ABENDDIC_VIEWS.html), are located in the same namespace which includes all [global types](ABENGLOBAL_TYPE_GLOSRY.html) of an AS ABAP. [DDIC domains](ABENDOMAIN_GLOSRY.html), on the other hand, are in a separate namespace, and can therefore have the same name as, for example, a data element.
...
-   DDIC types are stored in uppercase letters, as all [repository objects](ABENREPOSITORY_OBJECT_GLOSRY.html) of the [ABAP repository](ABENABAP_REPOSITORY_GLOSRY.html). Names that are entered in lowercase or mixed case letters in the [tools for dictionary objects](ABENDDIC_TOOLS.html) are converted to upper case internally.
-   In addition to the above rules, there are special

-   [naming rules for DDIC structures](ABENDDIC_STRUCTURES_NAMES.html)
-   [naming rules for DDIC database tables](ABENDDIC_DBTAB_NAMES.html)
-   [naming rules for DDIC views](ABENDDIC_VIEW_NAMES.html)
