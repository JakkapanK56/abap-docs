...
description: |
  ABENBDL_FIELD_CHAR - Standard ABAP language reference documentation
library: "standard"
...
`... field(readonly) Field1, Field2, ...;         | (mandatory) Field1, Field2, ...;         | (suppress) Field1, Field2, ...;         | (features:instance) Field1, Field2, ...;         | (mandatory:create) Field1, Field2, ...;         | (readonly:update) Field1, Field2, ...;         | (notrigger[:warn]) Field1, Field2, ...;`
...
[7. `... field(notrigger[:warn]) Field1, Field2, ...`](#ABAP_VARIANT_7@1@)
...
-   Defines that the field in question must not be used as the trigger condition of a [RAP determination](ABENRAP_DETERMINATION_GLOSRY.html) or a [RAP validation](ABENRAP_VALIDATION_GLOSRY.html).
...
-   This field characteristic can be useful, for example, in [RAP extensibility](ABENRAP_EXTENSIBILITY_GLOSRY.html) scenarios. The original RAP BO can exclude certain fields from usage in trigger conditions and this applies to all [RAP BO extensions](ABENRAP_BO_EXTENSION_GLOSRY.html).
-   [Static field attribute](ABENRAP_STATIC_FIELD_CHAR_GLOSRY.html).
-   No implementation in the ABAP behavior pool required.
