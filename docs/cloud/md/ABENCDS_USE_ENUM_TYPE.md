...
description: |
  ABENCDS_USE_ENUM_TYPE - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENCDS_USE_ENUM_TYPE.htm"
...
-   To type another CDS enumerated type as described in the topic [CDS DDL - `DEFINE TYPE ENUM`](ABENCDS_DEFINE_ENUM_TYPE.html).
...
-   As an operand of an association `ON` condition of a CDS association and of a to-parent association in a CDS view entity, CDS abstract entity and CDS transactional query.
...
-   `WHERE` clause
-   `HAVING` clause
-   Association `ON` condition, except in `ON` conditions in CDS custom entities.
-   Join `ON` condition
...
-   CDS enumerated types cannot be used for typing in DDIC objects such as [DDIC data types](ABENDDIC_DATA_TYPES.html), DDIC views, and [DDIC extensions](ABENDDIC_EXTENSION_GLOSRY.html).
