...
description: |
  ABAPDERIVED_TYPES_PID - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "types"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABAPDERIVED_TYPES_PID.htm"
...
The following source code section is taken from here: `BP_DEMO_UMANAGED_ROOT_LATE_NUMCCIMP`. The global class of the [behavior pool](ABENBEHAVIOR_POOL_GLOSRY.html) is `BP_DEMO_UMANAGED_ROOT_LATE_NUM`. The code example demonstrates the component `%pid` in the context of a `create` method that is responsible for creating `%pid` and mapping it to [`%cid`](ABAPDERIVED_TYPES_CID.html) in an unmanaged scenario. See the executable program for this example in the topic [Using `%pid`](ABENDERIVED_TYPES_PID_ABEXA.html).
