...
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENCDS_900971148_ANNO.htm"
...
This is similar to FILTER but display hierarchy can't be changed at runtime. But the display hierarchy can still be deactivated and activated.

**Kind** [Framework-specific annotation](ABENFRMWRK_ANNOTATION_GLOSRY.html)\\ **Syntax** `AnalyticsDetails.query.displayHierarchy`\\ **Scope** `#ELEMENT`\\ **Type** `String(20)`\\ **Enums** `#FILTER, #FILTER_ONLY, #OFF, #ON`\\ **Default** `-`\\ **Text** `-`\\ **MDE** `-`\\ **API** `RELEASED_FOR_KEY_USER_APPS, RELEASED_FOR_SAP_CLOUD_PLATFORM`