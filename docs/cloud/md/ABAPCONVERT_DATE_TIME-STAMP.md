...
description: |
  ABAPCONVERT_DATE_TIME-STAMP - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABAPCONVERT_DATE_TIME-STAMP.htm"
...
-   `tz` expects a character-like data object that contains a [time zone](ABENTIME_ZONE_GLOSRY.html).

-   If `tz` is initial, no time shift is calculated when converting to the time stamp. In this case, `sy-subrc` is set to 4.
-   If the specified time zone is not found, `time_stamp` remains unchanged and `sy-subrc` is set to 8.
