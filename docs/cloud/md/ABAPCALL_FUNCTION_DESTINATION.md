...
description: |
  ABAPCALL_FUNCTION_DESTINATION - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABAPCALL_FUNCTION_DESTINATION.htm"
abapFile: "ABAPCALL_FUNCTION_DESTINATION.html"
keywords: ["do", "if", "data", "ABAPCALL", "FUNCTION", "DESTINATION"]
---

``CALL FUNCTION func DESTINATION dest [`parameter_list`](ABAPCALL_FUNCTION_DESTINATION_PARA.html).``

Synchronous call (sRFC). The addition `DESTINATION` is used to specify the destination in `dest`. `func` and `dest` expect character-like data objects. The calling program is continued after the `CALL FUNCTION` statement when the remotely called function has finished.
...
-   The synchronous RFC triggers a [database commit](ABENDATABASE_COMMIT_GLOSRY.html) in the calling program.
-   The statement initializes `sy-msg*` components.
