...
description: |
  ABENCDS_FUNC_CLIENT_SAFETY - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENCDS_FUNC_CLIENT_SAFETY.htm"
...
Table functions with the annotation `\ [ClientHandling.type](ABENCDS_613831553_ANNO.html)` with the value `#CLIENT_INDEPENDENT` are client-unsafe. This cannot be changed with the annotation `[ClientHandling.clientSafe](ABENCDS_1549616294_ANNO.html)`.
...
The following `SELECT` statement accesses the CDS table function `DEMO_CDS_TABFUNC_SESS_VAR_CS` that is defined with the annotation `[ClientHandling.clientSafe](ABENCDS_1549616294_ANNO.html)`. Without the addition `USING CLIENT` that cannot be used in [ABAP for Cloud Development](ABENABAP_FOR_CLOUD_DEV_GLOSRY.html), only data of the current client can be selected.

-   All used objects must be client-safe.
-   The table function must have the annotations `[ClientHandling.type](ABENCDS_613831553_ANNO.html)` with the value `#CLIENT_DEPENDENT` together with `[ClientHandling.algorithm](ABENCDS_1114072352_ANNO.html)` with the value `#SESSION_VARIABLE`.
