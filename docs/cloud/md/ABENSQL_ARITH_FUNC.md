...
description: |
  ABENSQL_ARITH_FUNC - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "database"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENSQL_ARITH_FUNC.htm"
abapFile: "ABENSQL_ARITH_FUNC.html"
keywords: ["select", "do", "while", "if", "case", "data", "types", "internal-table", "ABENSQL", "ARITH", "FUNC"]
---

`... func( arg1[, arg2] ... ) ...`

Calls a numeric function `func` as an [SQL expression](ABAPSQL_EXPR.html) or operand of an expression in ABAP SQL. The arguments `arg1`, `arg2`, ... of the function are specified as a comma-separated list in parentheses. A blank must be placed after the opening parenthesis and in front of the closing parenthesis.
...
Conversion of flight time from minutes to hours. Round distance with null.
...
-   If a `CHAR` field of variable length is used as an argument in the SQL function [`GREATEST`](ABENSQL_FUNCTIONS_NUMERIC.html) or [`LEAST`](ABENSQL_FUNCTIONS_NUMERIC.html), the inferred type has the maximum length.
-   The SQL function [`GREATEST`](ABENSQL_FUNCTIONS_NUMERIC.html) must not be confused with the aggregate function [`MAX`](ABENSQL_AGG_FUNC.html) and the SQL function [`LEAST`](ABENSQL_FUNCTIONS_NUMERIC.html) must not be confused with the aggregate function [`MIN`](ABENSQL_AGG_FUNC.html). While `MAX` and `MIN` determine the maximum and minimal value of a SQL expression in a row set, `GREATEST` and `LEAST` determine the largest and smallest value of a set of arguments in a row set.
...
\\
Respective decimal floating point type for decimal floating point argument. x [`GREATEST( sql_exp1, sql_exp2[, sql_expn] )`](ABENSQL_FUNCTIONS_NUMERIC.html) Largest value of a set of arguments. `INT1`, `INT2`, `INT4`, `INT8`, `DEC`, `DECFLOAT16`, `DECFLOAT34`, `FLTP`, `CHAR`, `SSTRING`, `DATN`, `DATS`, `TIMN`, `TIMS`,`ACCP`, `UTCLONG`, `NUMC`, `CLNT`, `LANG`, `CURR`, `CUKY`, `QUAN`, and `UNIT` Type of the argument. - [`LEAST( sql_exp1, sql_exp2[, sql_expn] )`](ABENSQL_FUNCTIONS_NUMERIC.html) Smallest value of a set of arguments. `INT1`, `INT2`, `INT4`, `INT8`, `DEC`, `DECFLOAT16`, `DECFLOAT34`, `FLTP`, `CHAR`, `SSTRING`, `DATN`, `DATS`, `TIMN`, `TIMS`,`ACCP`, `UTCLONG`, `NUMC`, `CLNT`, `LANG`, `CURR`, `CUKY`, `QUAN`, and `UNIT` Type of the argument. - [`MOD( sql_exp1,sql_exp2 )`](ABENSQL_FUNCTIONS_NUMERIC.html) Positive or negative integer remainder of the division of `sql_exp1` by `sql_exp2`\\ `INT1`, `INT2`, `INT4`, and `INT8` plus `DEC`, `CURR`, and `QUAN` without decimal places Type of the argument with the greatest value range. x [`ROUND( sql_exp,pos )`](ABENSQL_FUNCTIONS_NUMERIC.html) Rounded value of `sql_exp`. If `pos` is greater than 0, the value is rounded to the position `pos` on the right of the decimal separator. If this is not the case, position `abs(pos)`+1 to the left of the decimal separator is rounded. This results in a 0 if the number of places is not sufficient. `sql_exp`: `INT1`, `INT2`, `INT4`, `INT8`, `DEC`, `DECFLOAT16`, `DECFLOAT34`, `CURR`, and `QUAN` \\
\\
