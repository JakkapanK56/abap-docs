...
description: |
  ABENDDIC_DATABASE_TABLES_FORKEY - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "database"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENDDIC_DATABASE_TABLES_FORKEY.htm"
...
-   **Input check**
-   In the definition of the foreign key, input checks by means of foreign key dependencies can be switched on or off. Furthermore, the standard message can be replaced by a custom [message](ABENMESSAGE_GLOSRY.html). Any placeholders in the message are filled as follows when the message is sent: The first three placeholders are filled with the content of the foreign key fields that are assigned to the first three key fields of the check table after the client field. The fourth placeholder is filled with the name of the check table.
-   **Generic foreign key fields**
-   Foreign key fields can be flagged as *generic* in the definition of a foreign key. Generic foreign key fields are not checked against their check table in input checks.
-   **Constant values**
-   Foreign key fields can be given constant values in the definition of a foreign key. In input checks, the field of the check table assigned to a foreign key field with a constant value must have this constant value in the row determined by the remaining foreign key fields.
-   **Inheritance**
...
-   **Cardinality**
-   A [cardinality](ABENCARDINALITY_GLOSRY.html)\\ `n:m` can be defined for each foreign key. This cardinality mainly serves a documentation purpose.
