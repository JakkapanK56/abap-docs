...
description: |
  ABENCDS_1937921420_ANNO - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENCDS_1937921420_ANNO.htm"
...
-   No explicit actual parameter can be assigned in ABAP SQL to an input parameter to which the system field `sy-mandt` was assigned using `#CLIENT`. ABAP SQL always implicitly passes the ID of the [current client](ABENCURRENT_CLIENT_GLOSRY.html) in accordance with the nominal value of `sy-mandt` or from the clients specified using `USING CLIENT`. The obsolete addition `CLIENT SPECIFIED` cannot be specified when a CDS entity of this type is accessed.
