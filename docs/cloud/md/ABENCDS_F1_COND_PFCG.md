...
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENCDS_F1_COND_PFCG.htm"
abapFile: "ABENCDS_F1_COND_PFCG.html"
keywords: ["select", "update", "do", "if", "case", "try", "class", "data", "types", "ABENCDS", "COND", "PFCG"]
---
...
During update task processing, the predefined aspect `pfcg_auth` behaves as if the user has full authorization. This replicates the behavior of the classic [`AUTHORITY-CHECK`](ABAPAUTHORITY-CHECK.html) statement, which in this situation always returns `sy-subrc` = 0.
...
The optional addition `IN SCENARIO` can be used to apply the switchable authorization to an authorization object.
