...
description: |
  ABENCDS_F1_CUSTOM_ASSOCIATION - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENCDS_F1_CUSTOM_ASSOCIATION.htm"
abapFile: "ABENCDS_F1_CUSTOM_ASSOCIATION.html"
keywords: ["select", "if", "data", "ABENCDS", "CUSTOM", "ASSOCIATION"]
---
...
-   As relational operator, only the equal operator (=) is allowed.
-   The Boolean operator `AND` is allowed. `OR` and `NOT` are not allowed.
-   `lhs` must be a [field](ABENCDS_FIELD_V2.html) of one of the two data sources [`data_source`](ABENCDS_DATA_SOURCE_V2.html) of the CDS association.
-   `rhs` must be a [field](ABENCDS_FIELD_V2.html) of both data sources [`data_source`](ABENCDS_DATA_SOURCE_V2.html) of the CDS association.
-   [CDS enumerated elements](ABENCDS_ENUM_ELEMENT_GLOSRY.html) are not allowed as operands in the `ON` condition.
