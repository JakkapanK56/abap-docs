...
description: |
  ABENITAB_ACCESS - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "data-structures"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENITAB_ACCESS.htm"
abapFile: "ABENITAB_ACCESS.html"
keywords: ["select", "loop", "do", "if", "try", "catch", "data", "types", "internal-table", "field-symbol", "ABENITAB", "ACCESS"]
---

When internal tables are accessed, either the entire table, or individual lines can be addressed.

The table category and the table keys are significant when internal tables are processed:

In the following example, data is written to an internal table with ABAP SQL, sorting takes place, and reads are demonstrated with the statement `READ TABLE` and a [table expression](ABENTABLE_EXPRESSION_GLOSRY.html).
