...
category: "general"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENSQL_EXPR_AGGR_IN_EXPR_ABEXA.htm"
abapFile: "ABENSQL_EXPR_AGGR_IN_EXPR_ABEXA.html"
keywords: ["select", "insert", "delete", "do", "if", "method", "class", "data", "types", "ABENSQL", "EXPR", "AGGR", "ABEXA"]
---

This example demonstrates [aggregate expressions](ABAPSELECT_AGGREGATE.html) as an argument of [SQL expressions](ABAPSQL_EXPR.html).

The DDIC database table `DEMO_EXPRESSIONS` is filled with random values in the static constructor.

See also the executable example for [using SQL expression in aggregate expressions](ABENSQL_EXPR_IN_AGGREGATES_ABEXA.html). Here, the roles of the expressions are switched, and different results are produced.
...
-   The second `SELECT` statement performs a grouping by the columns `CHAR1` and `CHAR2`. For each group, aggregated values whose arguments are aggregate expressions are determined using [arithmetic expressions](ABENSQL_ARITH.html).
-   The third `SELECT` statement performs the same grouping as the second `SELECT` statement, but restricts the selected groups using a [`HAVING` clause](ABAPHAVING_CLAUSE.html) in which a condition is set on an [arithmetic expression](ABENSQL_ARITH.html) that has aggregate expressions as arguments.
