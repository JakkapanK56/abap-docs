---
title: "ABENCDS_COMPOSITION_V2"
description: |
  ABENCDS_COMPOSITION_V2 - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "cds"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABENCDS_COMPOSITION_V2.htm"
abapFile: "ABENCDS_COMPOSITION_V2.html"
keywords: ["select", "do", "if", "case", "data", "ABENCDS", "COMPOSITION"]
---

``... COMPOSITION [[`cardinality`](ABENCDS_CARDINALITY_V2.html)]\ [OF] target [AS _compos] ...``

Defines a [to-child association](ABENTO_CHILD_ASSOCIATION_GLOSRY.html) in [ABAP CDS](ABENABAP_CORE_DATA_SERVICES_GLOSRY.html). A to-child association is a specialized [CDS association](ABENCDS_ASSOCIATION_GLOSRY.html) that defines the current [CDS view entity](ABENCDS_V2_VIEW_GLOSRY.html) as [parent entity](ABENPARENT_ENTITY_GLOSRY.html) of the [composition target](ABENCOMPOSITION_TARGET_GLOSRY.html). The composition target is the [child entity](ABENCHILD_ENTITY_GLOSRY.html) and it must define a [to-parent association](ABENTO_PARENT_ASSOCIATION_GLOSRY.html) to its parent entity. [To-child associations](ABENTO_CHILD_ASSOCIATION_GLOSRY.html) and [to-parent associations](ABENTO_PARENT_ASSOCIATION_GLOSRY.html) together define [CDS compositions](ABENCDS_COMPOSITION_GLOSRY.html) that in turn define the [CDS composition tree](ABENCDS_COMPOSITION_TREE_GLOSRY.html) of a [RAP business object](ABENRAP_BO_GLOSRY.html) in the context of the [ABAP RESTful Application Programming Model](ABENARAP_GLOSRY.html).

A to-child association is defined in a [`SELECT` statement](ABENCDS_SELECT_STATEMENT_V2.html) of a CDS view entity using the keyword `COMPOSITION`. A to-child association associates the current CDS entity, as a [composition source](ABENCOMPOSITION_SOURCE_GLOSRY.html), with the [composition target](ABENCOMPOSITION_TARGET_GLOSRY.html)\\ `target` specified in the definition of the to-child association.

The child entity (composition target) must already have a [to-parent association](ABENCDS_TO_PARENT_ASSOC_V2.html) to the composition parent before a to-child association can be defined in the parent.
...
The CDS view entity `DEMO_SALES_CDS_SO_I_VE` shown below returns information about sales order items. It defines three associations, one of them being a to-child association. The composition target (i.e. the child entity) is the CDS view entity `DEMO_SALES_CDS_SO_I_SL_VE`.
...
-   For the name of the to-child association `AS _compos`, the same rules apply as for a regular CDS association, see topic [CDS DDL - CDS View Entity, `ASSOCIATION`](ABENCDS_SIMPLE_ASSOCIATION_V2.html).
-   The [cardinality](ABENCARDINALITY_GLOSRY.html) can be specified written in words or using numeric syntax, as described in topic [CDS DDL - CDS View Entity, cardinality](ABENCDS_CARDINALITY_V2.html). If the cardinality syntax written in words is used, the [source cardinality](ABENSOURCE_CARDINALITY_GLOSRY.html) must be `EXACT ONE`. It can also be left out and is then implicitly set to `EXACT ONE`.
...
-   The composition target can be another [CDS view entity](ABENCDS_V2_VIEW_GLOSRY.html), a [CDS DDIC-based view (obsolete)](ABENCDS_V1_VIEW_GLOSRY.html), an [abstract entity](ABENCDS_ABSTRACT_ENTITY_GLOSRY.html), or a [custom entity](ABENCDS_CUSTOM_ENTITY_GLOSRY.html).
...
-   The composition target cannot be specified using a [path expression](ABENCDS_PATH_EXPRESSION_V2.html). The following is **not** allowed:
...
-   This also applies to CDS view extensions. A field from a composition target cannot be added to a CDS view entity via [`EXTEND VIEW ENTITY`](ABENCDS_EXTEND_VIEW_ENTITY.html).
...
-   The use cases and generation of joins described in topic [`CDS DDL - CDS View Entity, Associations and Joins`](ABENCDS_ASSOC_JOIN_V2.html) also apply to to-child associations, with the exception of use cases 3 and 4.
...
-   Defining new to-child associations is not supported in views with [set operators](ABENCDS_SET_OPERATORS.html).
