---
title: "ABAPDERIVED_TYPES_ELEMENT"
description: |
  ABAPDERIVED_TYPES_ELEMENT - ABAP Cloud language reference documentation
library: "cloud"
libraryName: "ABAP Cloud"
category: "types"
type: "abap-reference"
sourceUrl: "https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABAPDERIVED_TYPES_ELEMENT.htm"
abapFile: "ABAPDERIVED_TYPES_ELEMENT.html"
keywords: ["loop", "do", "if", "method", "class", "data", "types", "ABAPDERIVED", "TYPES", "ELEMENT"]
---

`%element` is a component in [BDEF derived types](ABENRAP_DERIVED_TYPE_GLOSRY.html). It is used in the context of the `REPORTED`\\ [RAP response parameter](ABENRAP_RESPONSE_PARAM_GLOSRY.html).
...
The following source code section is taken from here: `BP_DEMO_MANAGED_ROOT_REPORTED=CCIMP`. The global class of the [behavior pool](ABENBEHAVIOR_POOL_GLOSRY.html) is `BP_DEMO_MANAGED_ROOT_REPORTED`. It demonstrates the component `%element` in the context of a method implementation for a [validation](ABENBDL_VALIDATIONS.html) that is responsible for checking field values of RAP BO instances before saving. See the executable program for this example further down.

The example [Example for Message-Related BDEF Derived Type Components](ABENDERIVED_TYPES_REPORTED_ABEXA.html) demonstrates the use of various BDEF derived type components including `%element` with a managed RAP BO.
