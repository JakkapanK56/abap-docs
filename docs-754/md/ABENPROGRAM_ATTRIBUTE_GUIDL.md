---
title: "Program Attributes - ABAP Keyword Documentation"
sourceUrl: "https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenprogram_attribute_guidl.htm"
...
-   ABAP language version
    for configuring the [ABAP language version](ABENABAP_VERSION_GLOSRY.md "Glossary Entry")
...
-   ABAP language version is [Standard ABAP (Unicode)](ABENUNICODE_PROGRAM_GLOSRY.md "Glossary Entry") or higher
...
-   When a new program is created, the ABAP Language Version attribute is already set to [Standard ABAP (Unicode)](ABENUNICODE_PROGRAM_GLOSRY.md "Glossary Entry") by default. This attribute must never be set to [Non-Unicode ABAP](ABENNON_UNICODE_PROGRAM_GLOSRY.md "Glossary Entry"). Only Unicode systems are supported in the current release, which means such programs can no longer be used. Higher [ABAP language versions](ABENABAP_VERSION_GLOSRY.md "Glossary Entry") are possible because they cover the Unicode checks.
...
From Release 740, SP05, the [strict modes](ABENOPENSQL_STRICT_MODES.md) in the ABAP SQL syntax check require programs with the ABAP language version [Standard ABAP (Unicode)](ABENUNICODE_PROGRAM_GLOSRY.md "Glossary Entry") in which the program attribute [fixed point arithmetic](ABENFIXED_POINT_ARITHMETIC_GLOSRY.md "Glossary Entry") is switched on.
...
This was only possible for programs in the obsolete ABAP language version [Non-Unicode ABAP](ABENNON_UNICODE_PROGRAM_GLOSRY.md "Glossary Entry"). Here an implicit [casting](ABENCASTING_GUIDL.md "Guideline") of the subarea is performed for type c. The result in the components depends on the alignment gaps, the internal presentation of numeric values (byte order), and the code page used. Therefore, the result is extremely platform-dependent. A live program must never contain this type of code. This type of code often produces data with errors or runtime errors that are difficult to trace.

The above code produces a syntax error when used in an ABAP program defined as [Standard ABAP (Unicode)](ABENUNICODE_PROGRAM_GLOSRY.md "Glossary Entry") or higher in the program attributes (in accordance with the above rule). Unwanted substring accesses are prohibited, just like any other unwanted accesses to structures or other parts of the working memory. If these accesses cannot be identified by the syntax check, a runtime error occurs with a descriptive short dump while the program is running.
