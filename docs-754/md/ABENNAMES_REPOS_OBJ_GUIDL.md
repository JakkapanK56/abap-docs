---
title: "Names of Repository Objects - ABAP Keyword Documentation"
...
The global types, namely the global [object types](ABENOBJECT_TYPE_GLOSRY.md "Glossary Entry") of the [class library](ABENCLASS_LIBRARY_GLOSRY.md "Glossary Entry") and the global [data types](ABENDATA_TYPE_GLOSRY.md "Glossary Entry") of [ABAP Dictionary](ABENABAP_DICTIONARY_GLOSRY.md "Glossary Entry") are in separate dedicated namespaces. They are always unique when used after REF TO.
...
These naming conventions distinguish [object types](ABENOBJECT_TYPE_GLOSRY.md "Glossary Entry") from the global [data types](ABENDATA_TYPE_GLOSRY.md "Glossary Entry") of [ABAP Dictionary](ABENABAP_DICTIONARY_GLOSRY.md "Glossary Entry"), with which they share a namespace.
...
The above rule supplements the global rule regarding [descriptive names](ABENTELLING_NAMES_GUIDL.md "Guideline"). With regard to technical information in repository object names, the same information stated in section on descriptive names also applies here (also refer to the discussion on prefixes and suffixes in [program-internal names](ABENPROG_INTERN_NAMES_GUIDL.md "Guideline")).

All development organizations are free to create own naming conventions on the basis of the above rule regarding [descriptive names](ABENTELLING_NAMES_GUIDL.md "Guideline"). However, note that the use of a namespace prefix only leaves limited space for the remaining name components. For example, the names of database tables are restricted to 16 characters. Therefore, the namespace prefix should not be too long. It should simply describe a product line using an abbreviation
