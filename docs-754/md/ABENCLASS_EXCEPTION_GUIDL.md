---
title: "Classic and Class-Based Exceptions - ABAP Keyword Documentation"
...
-   Classic Exceptions
    These exceptions can only be declared in the interfaces of methods or function modules using EXCEPTIONS and can be raised within such a procedure using the statements [RAISE](ABAPRAISE_EXCEPTION.md) or [MESSAGE RAISING](ABAPMESSAGE_RAISING.md). The procedure caller can use the addition EXCEPTIONS of the statements [meth( ... )](ABAPCALL_METHOD_STATIC_SHORT.md) or [CALL FUNCTION](ABAPCALL_FUNCTION.md) to assign return codes for the system field sy-subrc to the exceptions the caller wants to handle and evaluate them after the call.
...
The raising of class-based exceptions, however, results in a change of the program flow. They can either be handled directly or propagated upwards along the call hierarchy. In this way, not every procedure ([method](ABENFUNCT_MODULE_SUBROUTINE_GUIDL.md "Guideline")) has to consider every possible exception situation itself. This supports the [separation of concerns](ABENSEPERATION_CONCERNS_GUIDL.md "Guideline") within an application. Because the exception can be represented by an object of an exception class, this exception object can gather additional information about the exception situation and transport it to the handler. In contrast to classic exceptions, this can also include specific [exception texts](ABENEXCEPTION_TEXTS_GUIDL.md "Guideline").
